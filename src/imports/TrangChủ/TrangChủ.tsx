import svgPaths from "./svg-w4udy0o29t";
import imgRectangle from "./9bcd28711286f8b0a11eba35e487d427ec1d1d22.png";
import imgRectangle1 from "./a3c910d3a4bc17e99c6a2952f94bf23c640b26ab.png";
import imgImage2 from "./9ac6fcc140281dd805dccc0f924042a5d7d1df4a.png";
import imgImage3 from "./f8589e4ad0cf44d5165f6ae4f2172a442d3a4be0.png";
import imgImage4 from "./f6a0e8f89a2506369cac141e422af8e90660990c.png";
import imgImage from "./03bfe456e2b9fd56c7e50eeb72fd261ca8850d27.png";
import imgImage1 from "./7c6709f3126e07f6f0ec2dccf8b979dd611cf169.png";
import imgBackground from "./f9b8c513c6f1279c9130e7d7020620f5c3b0ccf8.png";
import imgImageSlider2 from "./df13719c35f9cbdfb26e8fb2d26062e21c41a16c.png";
import imgBrand08Png from "./21407c0b99c60fc29b014440add3de670551064d.png";
import imgBrand07Png from "./6c5c42a34c9eacc73993f9ce62835d62404f0ee2.png";
import imgBrand06Png from "./94e746b86164557af011cecad63e99e99da71cd8.png";
import imgBrand04Png from "./b829bd23eefb6caabe6f949498ce08fe1a2564e0.png";
import imgBrand03Png from "./89df456a3e100335b80d7ca7d7ad248f4cb071ae.png";
import imgBrand02Png from "./3d38157bbc53af033773543bf2860c55b8023f9d.png";
import imgBrand09Png from "./4605c17e4188257a178fa0dd32be611f7278a6d4.png";
import imgBanner from "./0bd1d8e55618da3864ed4e4557419bd6e6c99dad.png";
import imgBanner1 from "./e4be452edb340fc6729a30cad87aff9a00f208d9.png";
import imgAirJordanDmp1Retro from "./b957fac1f8d50131a15ad42f8f2b097046045ebd.png";
import imgSecondImageOfAirJordanDmp1Retro from "./4874dfb16d99bf2e9456ba848e707ad414d011bd.png";
import imgAirJordanDmp1Retro1 from "./fa25379e812d9dddfb49229861453670e8e99724.png";
import imgSecondImageOfAirJordanDmp1Retro1 from "./73fc0d2451347ee9bb5f678eff480891a836731a.png";
import imgAirJordanDmp1Retro2 from "./4b1c34b74d60936ad774c03b3c498bd652a41fe6.png";
import imgSecondImageOfAirJordanDmp1Retro2 from "./07efb7f17b073299b1a70210f5e78b1e05e14db4.png";
import imgAirJordanDmp1Retro3 from "./c48fa71271af6fd83d4ce1b3f7d28e23cbb78b8e.png";
import imgSecondImageOfAirJordanDmp1Retro3 from "./50d9d6e7f76ac128256ec9d7cda69e42629aa79d.png";
import imgBanner2 from "./55def639ae98599ce3bf06c93fa2c4578db9f4f4.png";
import imgBanner3 from "./8ef96f6374c5e1c00fba23973a1d4766640515a5.png";
import imgImageProrudct from "./a227bd67d26144ffe3090446250e00ab80fedf32.png";
import imgNikeAirMax89Es from "./df3056b8ac054a9393841c0264d36fc1088cde03.png";
import imgSecondImageOfNikeAirMax89Es from "./26872436964b54174675dc8fd3a167c8d6f84fb3.png";
import imgLink5SweetOutfitIdeasYoullWearWellBeyondValentinesDay from "./eb8c685892e0df295135da0f8c93c9ee6a1558fa.png";
import imgLink12MustHaveShoesEveryFashionLoverNeedsInTheirCloset from "./d9ba888d38f1fd476885363ec75226d6c8c6c0a0.png";
import imgLinkThisScandiInfluencersShoeCollaborationIsTheGiftThatKeepsOnGiving from "./6e27b05fe34db924f81cf038e980f311d6266030.png";
import imgLinkHowToBeGoodWithMoneyAccordingTo4FemaleFinancialExperts from "./e56eacf1af6a4d2f7b5da5aeabc2500a195eb06f.png";
import imgImageContact1Png from "./c2000d94d8e5d7dd1fac8c081b9ad2c8a35297d1.png";
import imgImageContact2Png from "./e2f0bd8f595acccbcbac188ec93ae759f0c4de7e.png";
import imgFigureImageContact3Png from "./f6a7586ab049f7e8f087fb60cf77c048db40a1e4.png";
import imgLogo from "./89309c4327d643be9d6885f84d12f69b214a62e7.png";

function Container4() {
  return (
    <div className="content-stretch flex items-center justify-center min-h-[30px] px-[13px] py-[5px] relative shrink-0" data-name="Container">
      <div className="relative shrink-0 size-[20px]" data-name="Rectangle">
        <img alt="" className="absolute inset-0 max-w-none object-cover pointer-events-none size-full" src={imgRectangle} />
      </div>
    </div>
  );
}

function Button() {
  return (
    <div className="content-stretch flex items-center justify-center relative shrink-0" data-name="Button">
      <Container4 />
      <div className="absolute bg-[#61ff00] right-[-3px] rounded-[9999px] size-[18px] top-[-5px]" data-name="Background" />
    </div>
  );
}

function Container3() {
  return (
    <div className="content-stretch flex items-center justify-center min-h-[46px] min-w-[46px] py-[8px] relative rounded-[46px] shrink-0" data-name="Container">
      <Button />
    </div>
  );
}

function Container2() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0" data-name="Container">
      <Container3 />
    </div>
  );
}

function Container1() {
  return (
    <div className="content-stretch flex items-center justify-center relative shrink-0" data-name="Container">
      <Container2 />
    </div>
  );
}

function Margin1() {
  return (
    <div className="content-stretch flex flex-col items-start pb-[30px] relative shrink-0" data-name="Margin">
      <Container1 />
    </div>
  );
}

function Container8() {
  return (
    <div className="-translate-y-1/2 absolute content-stretch flex items-center justify-center left-0 min-h-[30px] px-[13px] py-[5px] right-0 top-[calc(50%+5px)]" data-name="Container">
      <div className="relative shrink-0 size-[20px]" data-name="Rectangle">
        <img alt="" className="absolute inset-0 max-w-none object-cover pointer-events-none size-full" src={imgRectangle1} />
      </div>
    </div>
  );
}

function Container7() {
  return (
    <div className="h-[20px] relative shrink-0 w-[46px]" data-name="Container">
      <Container8 />
    </div>
  );
}

function LinkWishlist() {
  return (
    <div className="content-stretch flex items-center justify-center min-h-[46px] min-w-[46px] py-[13px] relative rounded-[46px] shrink-0" data-name="Link - wishlist">
      <Container7 />
    </div>
  );
}

function Container6() {
  return (
    <div className="content-stretch flex flex-col items-center relative shrink-0" data-name="Container">
      <LinkWishlist />
    </div>
  );
}

function Container5() {
  return (
    <div className="content-stretch flex items-center justify-center relative shrink-0" data-name="Container">
      <Container6 />
    </div>
  );
}

function Margin2() {
  return (
    <div className="content-stretch flex flex-col items-start pb-[30px] relative shrink-0" data-name="Margin">
      <Container5 />
    </div>
  );
}

function Container() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0" data-name="Container">
      <Margin1 />
      <Margin2 />
    </div>
  );
}

function Margin() {
  return (
    <div className="relative shrink-0" data-name="Margin">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid content-stretch flex flex-col items-start pt-[20px] relative size-full">
        <Container />
      </div>
    </div>
  );
}

function Link() {
  return (
    <div className="bg-[#eaeaea] content-stretch flex items-center justify-center relative rounded-[30px] shrink-0 size-[30px]" data-name="Link">
      <div className="relative shrink-0 size-[17px]" data-name="image 2">
        <img alt="" className="absolute inset-0 max-w-none object-cover pointer-events-none size-full" src={imgImage2} />
      </div>
    </div>
  );
}

function LinkMargin() {
  return (
    <div className="content-stretch flex flex-col h-[40px] items-start pb-[10px] relative shrink-0 w-[30px]" data-name="Link:margin">
      <Link />
    </div>
  );
}

function Link1() {
  return (
    <div className="bg-[#eaeaea] content-stretch flex items-center justify-center relative rounded-[30px] shrink-0 size-[30px]" data-name="Link">
      <div className="relative shrink-0 size-[13px]" data-name="image 3">
        <img alt="" className="absolute inset-0 max-w-none object-cover pointer-events-none size-full" src={imgImage3} />
      </div>
    </div>
  );
}

function LinkMargin1() {
  return (
    <div className="content-stretch flex flex-col h-[40px] items-start pb-[10px] relative shrink-0 w-[30px]" data-name="Link:margin">
      <Link1 />
    </div>
  );
}

function Link2() {
  return (
    <div className="bg-[#eaeaea] content-stretch flex items-center justify-center relative rounded-[30px] shrink-0 size-[30px]" data-name="Link">
      <div className="flex items-center justify-center relative shrink-0">
        <div className="-scale-y-100 flex-none">
          <div className="h-[15px] relative w-[14px]" data-name="image 4">
            <img alt="" className="absolute inset-0 max-w-none object-cover pointer-events-none size-full" src={imgImage4} />
          </div>
        </div>
      </div>
    </div>
  );
}

function LinkMargin2() {
  return (
    <div className="content-stretch flex flex-col h-[40px] items-start pb-[10px] relative shrink-0 w-[30px]" data-name="Link:margin">
      <Link2 />
    </div>
  );
}

function Container11() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative self-stretch shrink-0" data-name="Container">
      <LinkMargin />
      <LinkMargin1 />
      <LinkMargin2 />
    </div>
  );
}

function Container10() {
  return (
    <div className="content-stretch flex h-[140px] items-start pb-[20px] relative shrink-0" data-name="Container">
      <Container11 />
    </div>
  );
}

function Container9() {
  return (
    <div className="relative shrink-0" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid content-stretch flex items-center relative size-full">
        <Container10 />
      </div>
    </div>
  );
}

function VerticalBorder() {
  return (
    <div className="absolute content-stretch flex flex-col h-[673.6px] items-center justify-between pl-[0.8px] right-0 top-0 w-[100px]" data-name="VerticalBorder">
      <div aria-hidden="true" className="absolute border-[#e4e4e4] border-l-[0.8px] border-solid inset-0 pointer-events-none" />
      <Margin />
      <Container9 />
    </div>
  );
}

function Image() {
  return (
    <div className="h-[1000px] relative shrink-0 w-full" data-name="Image">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute h-full left-[-14.39%] max-w-none top-0 w-[128.78%]" src={imgImage} />
      </div>
    </div>
  );
}

function Container17() {
  return (
    <div className="absolute bottom-0 content-stretch flex flex-col items-start left-0 min-h-px opacity-0 top-0 w-[1421px]" data-name="Container">
      <Image />
    </div>
  );
}

function Image1() {
  return (
    <div className="h-[1000px] relative shrink-0 w-full" data-name="Image">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute h-full left-[-14.39%] max-w-none top-0 w-[128.78%]" src={imgImage1} />
      </div>
    </div>
  );
}

function Container18() {
  return (
    <div className="absolute bottom-0 content-stretch flex flex-col items-start left-0 min-h-px opacity-0 top-0 w-[1421px]" data-name="Container">
      <Image1 />
    </div>
  );
}

function ImageSlider() {
  return (
    <div className="absolute inset-[14.95%_18.64%_-2.34%_28.44%]" data-name="Image Slider 2">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgImageSlider2} />
      </div>
    </div>
  );
}

function Heading1() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0 w-full" data-name="Heading 3">
      <div className="flex flex-col font-['Oswald:Medium',sans-serif] font-medium justify-center leading-[0] relative shrink-0 text-[24px] text-black uppercase w-full">
        <p className="leading-[32px]">NEW ARRIVALS</p>
      </div>
    </div>
  );
}

function Container23() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0 w-full" data-name="Container">
      <div className="capitalize flex flex-col font-['Jost:Black',sans-serif] font-black justify-center leading-[0] relative shrink-0 text-[72px] text-black w-full">
        <p className="leading-[72px] mb-0">Mulberry Area</p>
        <p className="leading-[72px]">Sneaker</p>
      </div>
    </div>
  );
}

function Link3() {
  return (
    <div className="bg-[#60ff00] content-stretch flex items-center justify-center px-[49px] py-[21px] relative rounded-[3px] shrink-0" data-name="Link">
      <div className="flex flex-col font-['Oswald:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[14px] text-black text-center uppercase whitespace-nowrap">
        <p className="leading-[14px]">BUY NOW</p>
      </div>
    </div>
  );
}

function Container24() {
  return (
    <div className="content-stretch flex items-center pt-[16px] relative shrink-0 w-full" data-name="Container">
      <Link3 />
    </div>
  );
}

function Container22() {
  return (
    <div className="content-stretch flex flex-col gap-[24px] items-start relative shrink-0 w-full" data-name="Container">
      <Heading1 />
      <Container23 />
      <Container24 />
    </div>
  );
}

function Container21() {
  return (
    <div className="absolute content-stretch flex flex-col inset-[35.2%_45.78%_35.2%_4.22%] items-start" data-name="Container">
      <Container22 />
    </div>
  );
}

function Container20() {
  return (
    <div className="flex-[1_0_0] min-h-px relative w-full" data-name="Container">
      <ImageSlider />
      <Container21 />
    </div>
  );
}

function Background() {
  return (
    <div className="content-stretch flex flex-col h-[1000px] items-start justify-center relative shrink-0 w-full" data-name="Background">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute h-full left-[-14.39%] max-w-none top-0 w-[128.78%]" src={imgBackground} />
      </div>
      <Container20 />
    </div>
  );
}

function Container19() {
  return (
    <div className="absolute bottom-0 content-stretch flex flex-col items-start left-0 min-h-px top-0 w-[1421px]" data-name="Container">
      <Background />
    </div>
  );
}

function Container16() {
  return (
    <div className="h-[1000px] relative shrink-0 w-[4263px]" data-name="Container">
      <Container17 />
      <Container18 />
      <Container19 />
    </div>
  );
}

function Container15() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0 w-full" data-name="Container">
      <Container16 />
    </div>
  );
}

function Container27() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0" data-name="Container">
      <div className="flex flex-col font-['Oswald:Medium',sans-serif] font-medium justify-center leading-[0] relative shrink-0 text-[12px] text-black whitespace-nowrap">
        <p className="leading-[18px]">02</p>
      </div>
    </div>
  );
}

function HorizontalDivider() {
  return (
    <div className="bg-white content-stretch flex flex-[1_0_0] h-px items-center min-w-px relative" data-name="Horizontal Divider">
      <div className="bg-black h-[3px] relative shrink-0 w-[144.82px]" data-name="Background" />
    </div>
  );
}

function Container28() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0" data-name="Container">
      <div className="flex flex-col font-['Oswald:Medium',sans-serif] font-medium justify-center leading-[0] relative shrink-0 text-[12px] text-black whitespace-nowrap">
        <p className="leading-[18px]">03</p>
      </div>
    </div>
  );
}

function Container26() {
  return (
    <div className="content-stretch flex gap-[20px] items-center relative shrink-0 w-full" data-name="Container">
      <Container27 />
      <HorizontalDivider />
      <Container28 />
    </div>
  );
}

function Container25() {
  return (
    <div className="absolute bottom-[50px] content-stretch flex flex-col items-start right-[241.52px] w-[241.53px]" data-name="Container">
      <Container26 />
    </div>
  );
}

function Container14() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0 w-full" data-name="Container">
      <Container15 />
      <Container25 />
    </div>
  );
}

function Container13() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative self-stretch" data-name="Container">
      <Container14 />
    </div>
  );
}

function Section() {
  return (
    <div className="content-stretch flex h-[1000px] items-start justify-center relative shrink-0 w-full" data-name="Section">
      <Container13 />
    </div>
  );
}

function Brand08Png() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-08.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand08Png} />
      </div>
    </div>
  );
}

function Link4() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Link">
      <Brand08Png />
    </div>
  );
}

function Container35() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Link4 />
    </div>
  );
}

function Container34() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container35 />
    </div>
  );
}

function Brand07Png() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-07.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand07Png} />
      </div>
    </div>
  );
}

function Link5() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Link">
      <Brand07Png />
    </div>
  );
}

function Container37() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Link5 />
    </div>
  );
}

function Container36() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container37 />
    </div>
  );
}

function Brand06Png() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-06.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand06Png} />
      </div>
    </div>
  );
}

function Link6() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Link">
      <Brand06Png />
    </div>
  );
}

function Container39() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Link6 />
    </div>
  );
}

function Container38() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container39 />
    </div>
  );
}

function Brand04Png() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-04.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand04Png} />
      </div>
    </div>
  );
}

function Link7() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Link">
      <Brand04Png />
    </div>
  );
}

function Container41() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Link7 />
    </div>
  );
}

function Container40() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container41 />
    </div>
  );
}

function Brand03Png() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-03.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand03Png} />
      </div>
    </div>
  );
}

function Link8() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Link">
      <Brand03Png />
    </div>
  );
}

function Container43() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Link8 />
    </div>
  );
}

function Container42() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container43 />
    </div>
  );
}

function Brand02Png() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-02.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand02Png} />
      </div>
    </div>
  );
}

function Container46() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Container">
      <Brand02Png />
    </div>
  );
}

function Container45() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Container46 />
    </div>
  );
}

function Container44() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container45 />
    </div>
  );
}

function Brand09Png() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-09.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand09Png} />
      </div>
    </div>
  );
}

function Link9() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Link">
      <Brand09Png />
    </div>
  );
}

function Container48() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Link9 />
    </div>
  );
}

function Container47() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container48 />
    </div>
  );
}

function Brand08Png1() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-08.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand08Png} />
      </div>
    </div>
  );
}

function Link10() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Link">
      <Brand08Png1 />
    </div>
  );
}

function Container50() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Link10 />
    </div>
  );
}

function Container49() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container50 />
    </div>
  );
}

function Brand07Png1() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-07.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand07Png} />
      </div>
    </div>
  );
}

function Link11() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Link">
      <Brand07Png1 />
    </div>
  );
}

function Container52() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Link11 />
    </div>
  );
}

function Container51() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container52 />
    </div>
  );
}

function Brand06Png1() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-06.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand06Png} />
      </div>
    </div>
  );
}

function Link12() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Link">
      <Brand06Png1 />
    </div>
  );
}

function Container54() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Link12 />
    </div>
  );
}

function Container53() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container54 />
    </div>
  );
}

function Brand04Png1() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-04.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand04Png} />
      </div>
    </div>
  );
}

function Link13() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Link">
      <Brand04Png1 />
    </div>
  );
}

function Container56() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Link13 />
    </div>
  );
}

function Container55() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container56 />
    </div>
  );
}

function Brand03Png1() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-03.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand03Png} />
      </div>
    </div>
  );
}

function Link14() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Link">
      <Brand03Png1 />
    </div>
  );
}

function Container58() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Link14 />
    </div>
  );
}

function Container57() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container58 />
    </div>
  );
}

function Brand02Png1() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-02.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand02Png} />
      </div>
    </div>
  );
}

function Container61() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Container">
      <Brand02Png1 />
    </div>
  );
}

function Container60() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Container61 />
    </div>
  );
}

function Container59() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container60 />
    </div>
  );
}

function Brand09Png1() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-09.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand09Png} />
      </div>
    </div>
  );
}

function Link15() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Link">
      <Brand09Png1 />
    </div>
  );
}

function Container63() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Link15 />
    </div>
  );
}

function Container62() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container63 />
    </div>
  );
}

function Brand08Png2() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-08.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand08Png} />
      </div>
    </div>
  );
}

function Link16() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Link">
      <Brand08Png2 />
    </div>
  );
}

function Container65() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Link16 />
    </div>
  );
}

function Container64() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container65 />
    </div>
  );
}

function Brand07Png2() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-07.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand07Png} />
      </div>
    </div>
  );
}

function Link17() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Link">
      <Brand07Png2 />
    </div>
  );
}

function Container67() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Link17 />
    </div>
  );
}

function Container66() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container67 />
    </div>
  );
}

function Brand06Png2() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-06.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand06Png} />
      </div>
    </div>
  );
}

function Link18() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Link">
      <Brand06Png2 />
    </div>
  );
}

function Container69() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Link18 />
    </div>
  );
}

function Container68() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container69 />
    </div>
  );
}

function Brand04Png2() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-04.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand04Png} />
      </div>
    </div>
  );
}

function Link19() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Link">
      <Brand04Png2 />
    </div>
  );
}

function Container71() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Link19 />
    </div>
  );
}

function Container70() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container71 />
    </div>
  );
}

function Brand03Png2() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-03.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand03Png} />
      </div>
    </div>
  );
}

function Link20() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Link">
      <Brand03Png2 />
    </div>
  );
}

function Container73() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Link20 />
    </div>
  );
}

function Container72() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container73 />
    </div>
  );
}

function Brand02Png2() {
  return (
    <div className="relative shrink-0 size-[115px]" data-name="brand-02.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBrand02Png} />
      </div>
    </div>
  );
}

function Container76() {
  return (
    <div className="content-stretch flex items-center justify-center overflow-clip relative shrink-0" data-name="Container">
      <Brand02Png2 />
    </div>
  );
}

function Container75() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Container">
      <Container76 />
    </div>
  );
}

function Container74() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px px-[60px] relative self-stretch shrink-0 w-[235px]" data-name="Container">
      <Container75 />
    </div>
  );
}

function Container33() {
  return (
    <div className="absolute content-stretch flex h-[115px] items-start left-[-2350px] top-0 w-[4700px]" data-name="Container">
      <Container34 />
      <Container36 />
      <Container38 />
      <Container40 />
      <Container42 />
      <Container44 />
      <Container47 />
      <Container49 />
      <Container51 />
      <Container53 />
      <Container55 />
      <Container57 />
      <Container59 />
      <Container62 />
      <Container64 />
      <Container66 />
      <Container68 />
      <Container70 />
      <Container72 />
      <Container74 />
    </div>
  );
}

function Container32() {
  return (
    <div className="absolute h-[115px] left-[-60px] overflow-clip right-[-60px] top-0" data-name="Container">
      <Container33 />
    </div>
  );
}

function Container31() {
  return (
    <div className="h-[115px] overflow-clip relative shrink-0 w-full" data-name="Container">
      <Container32 />
    </div>
  );
}

function Container30() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative self-stretch" data-name="Container">
      <Container31 />
    </div>
  );
}

function Container29() {
  return (
    <div className="content-stretch flex h-[115px] items-start justify-center max-w-[1290px] relative shrink-0 w-full" data-name="Container">
      <Container30 />
    </div>
  );
}

function Section2() {
  return (
    <div className="flex-[1_0_0] min-w-px relative self-stretch" data-name="Section">
      <div className="content-stretch flex flex-col items-start px-[50.4px] relative size-full">
        <Container29 />
      </div>
    </div>
  );
}

function Section1() {
  return (
    <div className="content-stretch flex h-[265px] items-start justify-center pt-[150px] relative shrink-0 w-[1390.8px]" data-name="Section">
      <Section2 />
    </div>
  );
}

function Banner() {
  return (
    <div className="aspect-[710.4000244140625/465.8299865722656] relative shrink-0 w-full" data-name="banner">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBanner} />
      </div>
    </div>
  );
}

function Container82() {
  return (
    <div className="content-stretch flex flex-col items-start overflow-clip relative shrink-0 w-full" data-name="Container">
      <Banner />
    </div>
  );
}

function Container85() {
  return (
    <div className="h-[7.629px] relative shrink-0 w-[12.285px]" data-name="Container">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 12.2852 7.62891">
        <g id="Container">
          <path d={svgPaths.p3a632720} fill="var(--fill-0, white)" id="Symbol" />
        </g>
      </svg>
    </div>
  );
}

function Background1() {
  return (
    <div className="bg-black content-stretch flex items-center justify-center relative rounded-[9999px] shrink-0 size-[40px]" data-name="Background">
      <Container85 />
    </div>
  );
}

function Margin3() {
  return (
    <div className="content-stretch flex flex-col h-[40px] items-start pl-[20px] relative shrink-0 w-[60px]" data-name="Margin">
      <Background1 />
    </div>
  );
}

function Link21() {
  return (
    <div className="content-stretch flex items-center justify-center relative shrink-0" data-name="Link">
      <div className="flex flex-col font-['Oswald:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[16px] text-black text-center uppercase whitespace-nowrap">
        <p className="leading-[24px]">SHOP WOMEN’S</p>
      </div>
      <Margin3 />
    </div>
  );
}

function Container84() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0 w-full" data-name="Container">
      <Link21 />
    </div>
  );
}

function Container83() {
  return (
    <div className="absolute bottom-[40px] content-stretch flex flex-col items-start left-[40px] w-[670.4px]" data-name="Container">
      <Container84 />
    </div>
  );
}

function Container81() {
  return (
    <div className="content-stretch flex flex-col items-start justify-center relative shrink-0 w-full" data-name="Container">
      <Container82 />
      <Container83 />
    </div>
  );
}

function Container80() {
  return (
    <div className="content-stretch flex flex-col items-start justify-center relative shrink-0 w-full" data-name="Container">
      <Container81 />
    </div>
  );
}

function Container79() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col h-full items-start min-w-px relative" data-name="Container">
      <Container80 />
    </div>
  );
}

function Container78() {
  return (
    <div className="content-stretch flex flex-[1_0_0] items-start justify-center min-h-px min-w-px relative self-stretch" data-name="Container">
      <Container79 />
    </div>
  );
}

function Banner1() {
  return (
    <div className="aspect-[710.4000244140625/465.8299865722656] relative shrink-0 w-full" data-name="banner">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBanner1} />
      </div>
    </div>
  );
}

function Container90() {
  return (
    <div className="content-stretch flex flex-col items-start overflow-clip relative shrink-0 w-full" data-name="Container">
      <Banner1 />
    </div>
  );
}

function Container93() {
  return (
    <div className="h-[7.629px] relative shrink-0 w-[12.285px]" data-name="Container">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 12.2852 7.62891">
        <g id="Container">
          <path d={svgPaths.p3a632720} fill="var(--fill-0, black)" id="Symbol" />
        </g>
      </svg>
    </div>
  );
}

function Background2() {
  return (
    <div className="bg-white content-stretch flex items-center justify-center relative rounded-[9999px] shrink-0 size-[40px]" data-name="Background">
      <Container93 />
    </div>
  );
}

function Margin4() {
  return (
    <div className="content-stretch flex flex-col h-[40px] items-start pl-[20px] relative shrink-0 w-[60px]" data-name="Margin">
      <Background2 />
    </div>
  );
}

function Link22() {
  return (
    <div className="content-stretch flex gap-[0.01px] items-center justify-center relative shrink-0" data-name="Link">
      <div className="flex flex-col font-['Oswald:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[16px] text-center text-white uppercase whitespace-nowrap">
        <p className="leading-[24px]">SHOP MEN’S</p>
      </div>
      <Margin4 />
    </div>
  );
}

function Container92() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0 w-full" data-name="Container">
      <Link22 />
    </div>
  );
}

function Container91() {
  return (
    <div className="absolute bottom-[40px] content-stretch flex flex-col items-start left-[40px] w-[670.4px]" data-name="Container">
      <Container92 />
    </div>
  );
}

function Container89() {
  return (
    <div className="content-stretch flex flex-col items-start justify-center relative shrink-0 w-full" data-name="Container">
      <Container90 />
      <Container91 />
    </div>
  );
}

function Container88() {
  return (
    <div className="content-stretch flex flex-col items-start justify-center relative shrink-0 w-full" data-name="Container">
      <Container89 />
    </div>
  );
}

function Container87() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col h-full items-start min-w-px relative" data-name="Container">
      <Container88 />
    </div>
  );
}

function Container86() {
  return (
    <div className="content-stretch flex flex-[1_0_0] items-start justify-center min-h-px min-w-px relative self-stretch" data-name="Container">
      <Container87 />
    </div>
  );
}

function Container77() {
  return (
    <div className="content-stretch flex h-[465.83px] items-start relative shrink-0 w-full" data-name="Container">
      <Container78 />
      <Container86 />
    </div>
  );
}

function Section4() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative self-stretch" data-name="Section">
      <Container77 />
    </div>
  );
}

function Section3() {
  return (
    <div className="content-stretch flex h-[615.83px] items-start justify-center pt-[150px] relative shrink-0 w-full" data-name="Section">
      <Section4 />
    </div>
  );
}

function Heading() {
  return (
    <div className="content-stretch flex flex-col items-center pb-[0.6px] relative shrink-0 w-full" data-name="Heading 2">
      <div className="flex flex-col font-['Oswald:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#222] text-[30px] text-center uppercase whitespace-nowrap">
        <p className="leading-[30px]">RECOMMENDED FOR YOU</p>
      </div>
    </div>
  );
}

function Container98() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative" data-name="Container">
      <Heading />
    </div>
  );
}

function Container97() {
  return (
    <div className="content-stretch flex h-[30px] items-center justify-center pb-[0.6px] relative shrink-0 w-full" data-name="Container">
      <Container98 />
    </div>
  );
}

function Container96() {
  return (
    <div className="absolute content-stretch flex flex-col items-center left-0 right-0 top-0" data-name="Container">
      <Container97 />
    </div>
  );
}

function AirJordanDmp1Retro() {
  return (
    <div className="aspect-[302.5/353.92999267578125] relative shrink-0 w-full" data-name="Air Jordan DMP 1 Retro">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgAirJordanDmp1Retro} />
      </div>
    </div>
  );
}

function SecondImageOfAirJordanDmp1Retro() {
  return (
    <div className="absolute h-[353.93px] left-0 max-w-[1536px] opacity-0 top-0 w-[302.5px]" data-name="Second image of Air Jordan DMP 1 Retro">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute h-[363.63%] left-[-131.82%] max-w-none top-[-131.82%] w-[363.64%]" src={imgSecondImageOfAirJordanDmp1Retro} />
      </div>
    </div>
  );
}

function Link23() {
  return (
    <div className="content-stretch flex flex-col items-start overflow-clip relative shrink-0 w-full" data-name="Link">
      <AirJordanDmp1Retro />
      <SecondImageOfAirJordanDmp1Retro />
    </div>
  );
}

function Container104() {
  return (
    <div className="absolute bottom-[15px] content-stretch flex flex-col items-center justify-center pb-[40px] right-[15px] w-[40px]" data-name="Container">
      <div className="h-[45px] relative shrink-0 w-[40px]" data-name="Rectangle" />
      <div className="h-[45px] relative shrink-0 w-[40px]" data-name="Rectangle" />
      <div className="h-[45px] relative shrink-0 w-[40px]" data-name="Rectangle" />
    </div>
  );
}

function Container103() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0 w-full" data-name="Container">
      <Link23 />
      <Container104 />
    </div>
  );
}

function Link24() {
  return (
    <div className="content-stretch flex flex-col items-start overflow-clip relative shrink-0 w-full" data-name="Link">
      <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#222] text-[14px] w-full">
        <p className="leading-[21px]">Calvin Klein Jeans</p>
      </div>
    </div>
  );
}

function ImgRated0OutOf() {
  return (
    <div className="h-[14.4px] relative shrink-0 w-[86.4px] z-[2]" data-name="Img - Rated 0 out of 5">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 86.4 14.4">
        <g id="Img - Rated 0 out of 5">
          <path d={svgPaths.p1caefc00} fill="var(--fill-0, #C3C3C3)" id="Symbol" />
        </g>
      </svg>
    </div>
  );
}

function Margin5() {
  return (
    <div className="content-stretch flex flex-col items-start pl-[4px] relative shrink-0 z-[1]" data-name="Margin">
      <div className="flex flex-col font-['Jost:Medium',sans-serif] font-medium justify-center leading-[0] relative shrink-0 text-[#b5b5b5] text-[12px] whitespace-nowrap">
        <p className="leading-[24px]">(0)</p>
      </div>
    </div>
  );
}

function Container106() {
  return (
    <div className="content-stretch flex isolate items-center relative shrink-0 w-full" data-name="Container">
      <ImgRated0OutOf />
      <Margin5 />
    </div>
  );
}

function Container107() {
  return (
    <div className="relative shrink-0 w-full" data-name="Container">
      <div className="content-stretch flex flex-col items-start pb-[3.4px] pl-[3px] pr-[249.1px] pt-[2.6px] relative size-full">
        <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#232630] text-[14px] w-full">
          <p className="leading-[14px]">$90.00</p>
        </div>
      </div>
    </div>
  );
}

function Container105() {
  return (
    <div className="content-stretch flex flex-col gap-[14px] items-start relative shrink-0 w-full" data-name="Container">
      <Link24 />
      <Container106 />
      <Container107 />
    </div>
  );
}

function Container102() {
  return (
    <div className="content-stretch flex flex-col gap-[24px] items-start relative shrink-0 w-full" data-name="Container">
      <Container103 />
      <Container105 />
    </div>
  );
}

function Container101() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px relative w-full" data-name="Container">
      <div className="absolute bg-white h-[500.93px] left-[-15px] opacity-0 rounded-[3px] shadow-[0px_0px_10px_0px_rgba(0,0,0,0.15)] top-[-15px] w-[332.5px]" data-name="Background+Shadow" />
      <Container102 />
    </div>
  );
}

function AirJordanDmp1Retro1() {
  return (
    <div className="aspect-[302.5/353.92999267578125] relative shrink-0 w-full" data-name="Air Jordan DMP 1 Retro">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgAirJordanDmp1Retro1} />
      </div>
    </div>
  );
}

function SecondImageOfAirJordanDmp1Retro1() {
  return (
    <div className="absolute h-[353.93px] left-0 max-w-[1536px] opacity-0 top-0 w-[302.5px]" data-name="Second image of Air Jordan DMP 1 Retro">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute h-[363.63%] left-[-131.82%] max-w-none top-[-131.82%] w-[363.64%]" src={imgSecondImageOfAirJordanDmp1Retro1} />
      </div>
    </div>
  );
}

function Link25() {
  return (
    <div className="content-stretch flex flex-col items-start overflow-clip relative shrink-0 w-full" data-name="Link">
      <AirJordanDmp1Retro1 />
      <SecondImageOfAirJordanDmp1Retro1 />
    </div>
  );
}

function Container111() {
  return (
    <div className="absolute bottom-[15px] content-stretch flex flex-col items-center justify-center pb-[40px] right-[15px] w-[40px]" data-name="Container">
      <div className="h-[45px] relative shrink-0 w-[40px]" data-name="Rectangle" />
      <div className="h-[45px] relative shrink-0 w-[40px]" data-name="Rectangle" />
      <div className="h-[45px] relative shrink-0 w-[40px]" data-name="Rectangle" />
    </div>
  );
}

function Container110() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0 w-full" data-name="Container">
      <Link25 />
      <Container111 />
    </div>
  );
}

function Link26() {
  return (
    <div className="content-stretch flex flex-col items-start overflow-clip relative shrink-0 w-full" data-name="Link">
      <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#222] text-[14px] w-full">
        <p className="leading-[21px]">Converse Run Star Motion</p>
      </div>
    </div>
  );
}

function ImgRated0OutOf1() {
  return (
    <div className="h-[14.4px] relative shrink-0 w-[86.4px] z-[2]" data-name="Img - Rated 0 out of 5">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 86.4 14.4">
        <g id="Img - Rated 0 out of 5">
          <path d={svgPaths.p1caefc00} fill="var(--fill-0, #C3C3C3)" id="Symbol" />
        </g>
      </svg>
    </div>
  );
}

function Margin6() {
  return (
    <div className="content-stretch flex flex-col items-start pl-[4px] relative shrink-0 z-[1]" data-name="Margin">
      <div className="flex flex-col font-['Jost:Medium',sans-serif] font-medium justify-center leading-[0] relative shrink-0 text-[#b5b5b5] text-[12px] whitespace-nowrap">
        <p className="leading-[24px]">(2)</p>
      </div>
    </div>
  );
}

function Container113() {
  return (
    <div className="content-stretch flex isolate items-center relative shrink-0 w-full" data-name="Container">
      <ImgRated0OutOf1 />
      <Margin6 />
    </div>
  );
}

function Container114() {
  return (
    <div className="relative shrink-0 w-full" data-name="Container">
      <div className="content-stretch flex flex-col items-start pb-[3.4px] pl-[3px] pr-[250.39px] pt-[2.6px] relative size-full">
        <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#232630] text-[14px] w-full">
          <p className="leading-[14px]">$88.00</p>
        </div>
      </div>
    </div>
  );
}

function Container112() {
  return (
    <div className="content-stretch flex flex-col gap-[14px] items-start relative shrink-0 w-full" data-name="Container">
      <Link26 />
      <Container113 />
      <Container114 />
    </div>
  );
}

function Container109() {
  return (
    <div className="content-stretch flex flex-col gap-[23.99px] items-start relative shrink-0 w-full" data-name="Container">
      <Container110 />
      <Container112 />
    </div>
  );
}

function Container108() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px relative w-full" data-name="Container">
      <div className="absolute bg-white h-[500.93px] left-[-15px] opacity-0 rounded-[3px] shadow-[0px_0px_10px_0px_rgba(0,0,0,0.15)] top-[-15px] w-[332.5px]" data-name="Background+Shadow" />
      <Container109 />
    </div>
  );
}

function AirJordanDmp1Retro2() {
  return (
    <div className="aspect-[302.5/353.92999267578125] relative shrink-0 w-full" data-name="Air Jordan DMP 1 Retro">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgAirJordanDmp1Retro2} />
      </div>
    </div>
  );
}

function SecondImageOfAirJordanDmp1Retro2() {
  return (
    <div className="absolute h-[353.93px] left-0 max-w-[1536px] opacity-0 top-0 w-[302.5px]" data-name="Second image of Air Jordan DMP 1 Retro">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute h-[363.63%] left-[-131.82%] max-w-none top-[-131.82%] w-[363.64%]" src={imgSecondImageOfAirJordanDmp1Retro2} />
      </div>
    </div>
  );
}

function Link27() {
  return (
    <div className="content-stretch flex flex-col items-start overflow-clip relative shrink-0 w-full" data-name="Link">
      <AirJordanDmp1Retro2 />
      <SecondImageOfAirJordanDmp1Retro2 />
    </div>
  );
}

function Container118() {
  return (
    <div className="absolute bottom-[15px] content-stretch flex flex-col items-center justify-center pb-[40px] right-[15px] w-[40px]" data-name="Container">
      <div className="h-[45px] relative shrink-0 w-[40px]" data-name="Rectangle" />
      <div className="h-[45px] relative shrink-0 w-[40px]" data-name="Rectangle" />
      <div className="h-[45px] relative shrink-0 w-[40px]" data-name="Rectangle" />
    </div>
  );
}

function Container117() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0 w-full" data-name="Container">
      <Link27 />
      <Container118 />
    </div>
  );
}

function Link28() {
  return (
    <div className="content-stretch flex flex-col items-start overflow-clip relative shrink-0 w-full" data-name="Link">
      <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#222] text-[14px] w-full">
        <p className="leading-[21px]">{`Canvas Men's Sneakers`}</p>
      </div>
    </div>
  );
}

function ImgRated0OutOf2() {
  return (
    <div className="h-[14.4px] relative shrink-0 w-[86.4px] z-[2]" data-name="Img - Rated 0 out of 5">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 86.4 14.4">
        <g id="Img - Rated 0 out of 5">
          <path d={svgPaths.p1caefc00} fill="var(--fill-0, #C3C3C3)" id="Symbol" />
        </g>
      </svg>
    </div>
  );
}

function Margin7() {
  return (
    <div className="content-stretch flex flex-col items-start pl-[4px] relative shrink-0 z-[1]" data-name="Margin">
      <div className="flex flex-col font-['Jost:Medium',sans-serif] font-medium justify-center leading-[0] relative shrink-0 text-[#b5b5b5] text-[12px] whitespace-nowrap">
        <p className="leading-[24px]">(0)</p>
      </div>
    </div>
  );
}

function Container120() {
  return (
    <div className="content-stretch flex isolate items-center relative shrink-0 w-full" data-name="Container">
      <ImgRated0OutOf2 />
      <Margin7 />
    </div>
  );
}

function Container121() {
  return (
    <div className="relative shrink-0 w-full" data-name="Container">
      <div className="content-stretch flex flex-col items-start pb-[3.4px] pl-[3px] pr-[249.59px] pt-[2.6px] relative size-full">
        <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#232630] text-[14px] w-full">
          <p className="leading-[14px]">$86.00</p>
        </div>
      </div>
    </div>
  );
}

function Container119() {
  return (
    <div className="content-stretch flex flex-col gap-[14px] items-start relative shrink-0 w-full" data-name="Container">
      <Link28 />
      <Container120 />
      <Container121 />
    </div>
  );
}

function Container116() {
  return (
    <div className="content-stretch flex flex-col gap-[24px] items-start relative shrink-0 w-full" data-name="Container">
      <Container117 />
      <Container119 />
    </div>
  );
}

function Container115() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px relative w-full" data-name="Container">
      <div className="absolute bg-white h-[500.93px] left-[-15px] opacity-0 rounded-[3px] shadow-[0px_0px_10px_0px_rgba(0,0,0,0.15)] top-[-15px] w-[332.5px]" data-name="Background+Shadow" />
      <Container116 />
    </div>
  );
}

function AirJordanDmp1Retro3() {
  return (
    <div className="aspect-[302.5/353.92999267578125] relative shrink-0 w-full" data-name="Air Jordan DMP 1 Retro">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgAirJordanDmp1Retro3} />
      </div>
    </div>
  );
}

function SecondImageOfAirJordanDmp1Retro3() {
  return (
    <div className="absolute h-[353.93px] left-0 max-w-[1536px] opacity-0 top-0 w-[302.5px]" data-name="Second image of Air Jordan DMP 1 Retro">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute h-[363.63%] left-[-131.82%] max-w-none top-[-131.82%] w-[363.64%]" src={imgSecondImageOfAirJordanDmp1Retro3} />
      </div>
    </div>
  );
}

function Link29() {
  return (
    <div className="content-stretch flex flex-col items-start overflow-clip relative shrink-0 w-full" data-name="Link">
      <AirJordanDmp1Retro3 />
      <SecondImageOfAirJordanDmp1Retro3 />
    </div>
  );
}

function Container125() {
  return (
    <div className="absolute bottom-[15px] content-stretch flex flex-col items-start right-[15px] w-[40px]" data-name="Container">
      <div className="h-[45px] relative shrink-0 w-full" data-name="Rectangle" />
      <div className="h-[45px] relative shrink-0 w-[40px]" data-name="Rectangle" />
      <div className="h-[45px] relative shrink-0 w-[40px]" data-name="Rectangle" />
    </div>
  );
}

function Container124() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0 w-full" data-name="Container">
      <Link29 />
      <Container125 />
    </div>
  );
}

function Link30() {
  return (
    <div className="content-stretch flex flex-col items-start overflow-clip relative shrink-0 w-full" data-name="Link">
      <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#222] text-[14px] w-full">
        <p className="leading-[21px]">Jordan Luka 1 Basketball</p>
      </div>
    </div>
  );
}

function ImgRated0OutOf3() {
  return (
    <div className="h-[14.4px] relative shrink-0 w-[86.4px] z-[2]" data-name="Img - Rated 0 out of 5">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 86.4 14.4">
        <g id="Img - Rated 0 out of 5">
          <path d={svgPaths.p1caefc00} fill="var(--fill-0, #C3C3C3)" id="Symbol" />
        </g>
      </svg>
    </div>
  );
}

function Margin8() {
  return (
    <div className="content-stretch flex flex-col items-start pl-[4px] relative shrink-0 z-[1]" data-name="Margin">
      <div className="flex flex-col font-['Jost:Medium',sans-serif] font-medium justify-center leading-[0] relative shrink-0 text-[#b5b5b5] text-[12px] whitespace-nowrap">
        <p className="leading-[24px]">(0)</p>
      </div>
    </div>
  );
}

function Container127() {
  return (
    <div className="content-stretch flex isolate items-center relative shrink-0 w-full" data-name="Container">
      <ImgRated0OutOf3 />
      <Margin8 />
    </div>
  );
}

function Container128() {
  return (
    <div className="relative shrink-0 w-full" data-name="Container">
      <div className="content-stretch flex flex-col items-start pb-[3.4px] pl-[3px] pr-[249.19px] pt-[2.6px] relative size-full">
        <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#7c7c7c] text-[14px] w-full">
          <p>
            <span className="leading-[14px]">$</span>
            <span className="[text-decoration-skip-ink:none] decoration-solid leading-[14px] line-through">90.00</span>
          </p>
        </div>
      </div>
    </div>
  );
}

function Container126() {
  return (
    <div className="content-stretch flex flex-col gap-[14px] items-start relative shrink-0 w-full" data-name="Container">
      <Link30 />
      <Container127 />
      <Container128 />
    </div>
  );
}

function Container123() {
  return (
    <div className="content-stretch flex flex-col gap-[23.99px] items-start relative shrink-0 w-full" data-name="Container">
      <Container124 />
      <Container126 />
    </div>
  );
}

function Container122() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px relative w-full" data-name="Container">
      <div className="absolute bg-white h-[500.93px] left-[-15px] opacity-0 rounded-[3px] shadow-[0px_0px_10px_0px_rgba(0,0,0,0.15)] top-[-15px] w-[332.5px]" data-name="Background+Shadow" />
      <Container123 />
    </div>
  );
}

function Container100() {
  return (
    <div className="content-stretch flex h-[615px] items-center justify-center overflow-clip relative shrink-0 w-full" data-name="Container">
      <div className="content-stretch flex flex-col h-[430.93px] items-start justify-center pb-[30px] px-[15px] relative shrink-0 w-[332.5px]" data-name="Sản phẩm">
        <Container101 />
      </div>
      <div className="content-stretch flex flex-col h-[430.93px] items-start justify-center pb-[30px] px-[15px] relative shrink-0 w-[332.5px]" data-name="Sản phẩm">
        <Container108 />
      </div>
      <div className="content-stretch flex flex-col h-[430.93px] items-start justify-center pb-[30px] px-[15px] relative shrink-0 w-[332.5px]" data-name="Sản phẩm">
        <Container115 />
      </div>
      <div className="content-stretch flex flex-col h-[430.93px] items-start justify-center pb-[30px] px-[15px] relative shrink-0 w-[332.5px]" data-name="Sản phẩm">
        <Container122 />
      </div>
    </div>
  );
}

function Container99() {
  return (
    <div className="absolute content-stretch flex flex-col h-[711px] items-start left-[9.6px] right-[-19.6px] top-[25.18px]" data-name="Container">
      <Container100 />
    </div>
  );
}

function Container95() {
  return (
    <div className="h-[659px] relative shrink-0 w-[1290px]" data-name="Container">
      <Container96 />
      <Container99 />
    </div>
  );
}

function Container132() {
  return (
    <div className="content-stretch flex flex-col items-center relative self-stretch shrink-0" data-name="Container">
      <div className="flex flex-col font-['Oswald:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[12px] text-black text-center uppercase whitespace-nowrap">
        <p className="leading-[18px]">VIEW ALL PRODUCT</p>
      </div>
    </div>
  );
}

function Container131() {
  return (
    <div className="content-stretch flex h-[18px] items-start justify-center relative shrink-0" data-name="Container">
      <Container132 />
    </div>
  );
}

function Link31() {
  return (
    <div className="bg-[#f2f2f2] content-stretch flex items-start px-[55px] py-[18px] relative rounded-[3px] shrink-0" data-name="Link">
      <Container131 />
    </div>
  );
}

function Container130() {
  return (
    <div className="content-stretch flex flex-col items-center relative shrink-0 w-full" data-name="Container">
      <Link31 />
    </div>
  );
}

function Container129() {
  return (
    <div className="content-stretch flex flex-col h-[134px] items-start pt-[80px] relative shrink-0 w-full" data-name="Container">
      <Container130 />
    </div>
  );
}

function Container94() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start justify-center min-w-px relative self-stretch" data-name="Container">
      <Container95 />
      <Container129 />
    </div>
  );
}

function Section5() {
  return (
    <div className="content-stretch flex h-[932.99px] items-start justify-center max-w-[1290px] pt-[139.99px] relative shrink-0 w-[1290px]" data-name="Section">
      <Container94 />
    </div>
  );
}

function Banner2() {
  return (
    <div className="h-[621.11px] max-w-[710.4000244140625px] relative shrink-0 w-[710.4px]" data-name="banner">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBanner2} />
      </div>
    </div>
  );
}

function Link32() {
  return (
    <div className="content-stretch flex items-start justify-center relative shrink-0" data-name="Link">
      <Banner2 />
    </div>
  );
}

function Container134() {
  return (
    <div className="col-[1/span_6] content-stretch flex flex-col items-center justify-self-stretch overflow-clip relative row-1 self-start shrink-0" data-name="Container">
      <Link32 />
    </div>
  );
}

function Container137() {
  return (
    <div className="content-stretch flex flex-col items-center pb-[0.7px] relative shrink-0 w-full" data-name="Container">
      <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#909090] text-[18px] text-center uppercase whitespace-nowrap">
        <p className="leading-[27px]">RUNNING SHOES</p>
      </div>
    </div>
  );
}

function Container139() {
  return (
    <div className="content-stretch flex flex-col items-center px-[9.07px] relative shrink-0" data-name="Container">
      <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[24px] text-black text-center whitespace-nowrap">
        <p className="leading-[32px] mb-0">{`Our Mate men's running shoes are made`}</p>
        <p className="leading-[32px] mb-0">to give you perfect balance and boost</p>
        <p className="leading-[32px]">while out for a run</p>
      </div>
    </div>
  );
}

function Container138() {
  return (
    <div className="content-stretch flex items-center justify-center pb-[20px] relative shrink-0 w-[470px]" data-name="Container">
      <Container139 />
    </div>
  );
}

function Link33() {
  return (
    <div className="bg-black content-stretch flex items-center justify-center px-[45px] py-[15px] relative rounded-[3px] shrink-0" data-name="Link">
      <div className="flex flex-col font-['Oswald:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[14px] text-center text-white uppercase whitespace-nowrap">
        <p className="leading-[24px]">SHOP NOW</p>
      </div>
    </div>
  );
}

function Container136() {
  return (
    <div className="content-stretch flex flex-col gap-[20px] items-center relative shrink-0" data-name="Container">
      <Container137 />
      <Container138 />
      <Link33 />
    </div>
  );
}

function Container135() {
  return (
    <div className="col-[7/span_6] h-[621.11px] justify-self-stretch relative row-1 shrink-0" data-name="Container">
      <div className="flex flex-row items-center justify-center size-full">
        <div className="content-stretch flex items-center justify-center pb-[192.06px] pt-[191.35px] px-[15px] relative size-full">
          <Container136 />
        </div>
      </div>
    </div>
  );
}

function Background3() {
  return (
    <div className="bg-[#f0f0f0] grid grid-cols-[repeat(12,minmax(0,1fr))] grid-rows-[_621.11px] relative shrink-0 w-full" data-name="Background">
      <Container134 />
      <Container135 />
    </div>
  );
}

function Container133() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative self-stretch" data-name="Container">
      <Background3 />
    </div>
  );
}

function Section6() {
  return (
    <div className="content-stretch flex h-[761.112px] items-start justify-center pt-[140px] relative shrink-0 w-full" data-name="Section">
      <Container133 />
    </div>
  );
}

function Container143() {
  return (
    <div className="content-stretch flex flex-col items-center pb-[0.7px] relative shrink-0 w-full" data-name="Container">
      <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#909090] text-[18px] text-center uppercase whitespace-nowrap">
        <p className="leading-[27px]">BASKETBALL SHOES</p>
      </div>
    </div>
  );
}

function Container145() {
  return (
    <div className="content-stretch flex flex-col items-center pl-[18.8px] pr-[18.81px] relative shrink-0" data-name="Container">
      <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[24px] text-black text-center whitespace-nowrap">
        <p className="leading-[32px] mb-0">The best beginner rock climbing shoes</p>
        <p className="leading-[32px]">for bouldering, gym and outdoor use</p>
      </div>
    </div>
  );
}

function Container144() {
  return (
    <div className="content-stretch flex items-center justify-center pb-[20px] relative shrink-0 w-[470px]" data-name="Container">
      <Container145 />
    </div>
  );
}

function Link34() {
  return (
    <div className="bg-black content-stretch flex items-center justify-center px-[45px] py-[15px] relative rounded-[3px] shrink-0" data-name="Link">
      <div className="flex flex-col font-['Oswald:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[14px] text-center text-white uppercase whitespace-nowrap">
        <p className="leading-[24px]">SHOP NOW</p>
      </div>
    </div>
  );
}

function Container142() {
  return (
    <div className="content-stretch flex flex-col gap-[20px] items-center relative shrink-0" data-name="Container">
      <Container143 />
      <Container144 />
      <Link34 />
    </div>
  );
}

function Container141() {
  return (
    <div className="col-[1/span_6] h-[621.11px] justify-self-stretch relative row-1 shrink-0" style={{ backgroundImage: "linear-gradient(90deg, rgba(0, 0, 0, 0.2) 0%, rgba(0, 0, 0, 0.2) 100%), linear-gradient(90deg, rgb(255, 255, 255) 0%, rgb(255, 255, 255) 100%)" }} data-name="Container">
      <div className="flex flex-row items-center justify-center size-full">
        <div className="content-stretch flex items-center justify-center pb-[208.06px] pt-[207.35px] px-[15px] relative size-full">
          <Container142 />
        </div>
      </div>
    </div>
  );
}

function Banner3() {
  return (
    <div className="h-[621.11px] max-w-[710.4000244140625px] relative shrink-0 w-[710.4px]" data-name="banner">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgBanner3} />
      </div>
    </div>
  );
}

function Link35() {
  return (
    <div className="content-stretch flex items-start justify-center relative shrink-0" data-name="Link">
      <Banner3 />
    </div>
  );
}

function Container146() {
  return (
    <div className="col-[7/span_6] content-stretch flex flex-col items-center justify-self-stretch overflow-clip relative row-1 self-start shrink-0" data-name="Container">
      <Link35 />
    </div>
  );
}

function Background4() {
  return (
    <div className="bg-[#f0f0f0] grid grid-cols-[repeat(12,minmax(0,1fr))] grid-rows-[_621.11px] relative shrink-0 w-full" data-name="Background">
      <Container141 />
      <Container146 />
    </div>
  );
}

function Container140() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative self-stretch" data-name="Container">
      <Background4 />
    </div>
  );
}

function Section7() {
  return (
    <div className="content-stretch flex h-[621.112px] items-start justify-center relative shrink-0 w-full" data-name="Section">
      <Container140 />
    </div>
  );
}

function ImageProrudct() {
  return (
    <div className="aspect-[710.4000244140625/621.1099853515625] col-[1/span_6] justify-self-stretch relative row-1 shrink-0" data-name="image-prorudct">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgImageProrudct} />
      </div>
    </div>
  );
}

function NikeAirMax89Es() {
  return (
    <div className="aspect-[306/358.010009765625] relative shrink-0 w-full" data-name="Nike Air Max 89 ES">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgNikeAirMax89Es} />
      </div>
    </div>
  );
}

function SecondImageOfNikeAirMax89Es() {
  return (
    <div className="absolute h-[358.01px] left-0 max-w-[1536px] opacity-0 top-0 w-[305.99px]" data-name="Second image of Nike Air Max 89 ES">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-[-129.74%] max-w-none size-[359.49%] top-[-129.74%]" src={imgSecondImageOfNikeAirMax89Es} />
      </div>
    </div>
  );
}

function Link36() {
  return (
    <div className="content-stretch flex flex-col items-start overflow-clip relative shrink-0 w-full" data-name="Link">
      <NikeAirMax89Es />
      <SecondImageOfNikeAirMax89Es />
    </div>
  );
}

function Container155() {
  return (
    <div className="absolute bottom-[15px] content-stretch flex flex-col items-center justify-center pb-[40px] right-[15px] w-[40px]" data-name="Container">
      <div className="h-[45px] relative shrink-0 w-[40px]" data-name="Rectangle" />
      <div className="h-[45px] relative shrink-0 w-[40px]" data-name="Rectangle" />
      <div className="h-[45px] relative shrink-0 w-[40px]" data-name="Rectangle" />
    </div>
  );
}

function Label() {
  return (
    <div className="bg-[#db0000] content-stretch flex flex-col items-center min-w-[50px] pl-[12.75px] pr-[12.76px] py-[4px] relative rounded-[3px] shrink-0" data-name="Label">
      <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[10px] text-center text-white uppercase whitespace-nowrap">
        <p className="leading-[15px]">-50%</p>
      </div>
    </div>
  );
}

function Container156() {
  return (
    <div className="absolute content-stretch flex items-center left-[10px] pl-[6px] top-[10px]" data-name="Container">
      <Label />
    </div>
  );
}

function Container154() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0 w-full" data-name="Container">
      <Link36 />
      <Container155 />
      <Container156 />
    </div>
  );
}

function Link37() {
  return (
    <div className="content-stretch flex flex-col items-center overflow-clip relative shrink-0 w-full" data-name="Link">
      <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#222] text-[14px] text-center whitespace-nowrap">
        <p className="leading-[21px]">Nike Air Max 89 ES</p>
      </div>
    </div>
  );
}

function Container159() {
  return (
    <div className="h-[14.4px] relative shrink-0 w-[83px]" data-name="Container">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 83 14.4">
        <g id="Container">
          <path d={svgPaths.p167edd40} fill="var(--fill-0, #C3C3C3)" id="Symbol" />
        </g>
      </svg>
    </div>
  );
}

function ImgRated0OutOf4() {
  return (
    <div className="content-stretch flex flex-col h-[14.4px] items-start overflow-clip relative shrink-0 w-[86.4px] z-[2]" data-name="Img - Rated 0 out of 5">
      <Container159 />
    </div>
  );
}

function Container160() {
  return (
    <div className="content-stretch flex flex-col items-center relative shrink-0" data-name="Container">
      <div className="flex flex-col font-['Jost:Medium',sans-serif] font-medium justify-center leading-[0] relative shrink-0 text-[#b5b5b5] text-[12px] text-center whitespace-nowrap">
        <p className="leading-[24px]">(0)</p>
      </div>
    </div>
  );
}

function Margin9() {
  return (
    <div className="content-stretch flex flex-col items-start pl-[4px] relative shrink-0 z-[1]" data-name="Margin">
      <Container160 />
    </div>
  );
}

function Container158() {
  return (
    <div className="content-stretch flex isolate items-center justify-center relative shrink-0 w-full" data-name="Container">
      <ImgRated0OutOf4 />
      <Margin9 />
    </div>
  );
}

function Container162() {
  return (
    <div className="h-[14px] relative shrink-0 w-[56.4px]" data-name="Container">
      <div className="-translate-x-1/2 -translate-y-1/2 absolute flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] left-1/2 text-[#232630] text-[14px] text-center top-[6.4px] whitespace-nowrap">
        <p className="leading-[14px]">$90.00</p>
      </div>
    </div>
  );
}

function Del() {
  return (
    <div className="content-stretch flex flex-col items-center pb-[3.4px] pt-[2.6px] px-[3px] relative shrink-0" data-name="Del">
      <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#7c7c7c] text-[14px] text-center whitespace-nowrap">
        <p>
          <span className="leading-[14px]">$</span>
          <span className="[text-decoration-skip-ink:none] decoration-solid font-['Jost:Bold',sans-serif] font-bold leading-[14px] line-through">180.00</span>
        </p>
      </div>
    </div>
  );
}

function DelMargin() {
  return (
    <div className="content-stretch flex flex-col items-start pl-[6px] relative shrink-0" data-name="Del:margin">
      <Del />
    </div>
  );
}

function Container161() {
  return (
    <div className="content-stretch flex items-center justify-center pt-[6px] relative shrink-0 w-full" data-name="Container">
      <Container162 />
      <DelMargin />
    </div>
  );
}

function Container157() {
  return (
    <div className="content-stretch flex flex-col gap-[8px] items-start relative shrink-0 w-full" data-name="Container">
      <Link37 />
      <Container158 />
      <Container161 />
    </div>
  );
}

function Container153() {
  return (
    <div className="content-stretch flex flex-col gap-[24px] items-start relative self-stretch shrink-0 w-[306px]" data-name="Container">
      <Container154 />
      <Container157 />
    </div>
  );
}

function Container152() {
  return (
    <div className="content-stretch flex h-[469.01px] items-start justify-center relative shrink-0 w-full" data-name="Container">
      <Container153 />
    </div>
  );
}

function Container151() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px relative shrink-0 w-[346px]" data-name="Container">
      <Container152 />
    </div>
  );
}

function Container150() {
  return (
    <div className="absolute content-stretch flex flex-col items-start justify-center left-[-20px] top-0 w-[346px]" data-name="Container">
      <Container151 />
    </div>
  );
}

function Container149() {
  return (
    <div className="h-[469.01px] max-w-[305.47198486328125px] overflow-clip relative shrink-0 w-[305.46px]" data-name="Container">
      <Container150 />
    </div>
  );
}

function Background5() {
  return (
    <div className="bg-[#f5f5f5] col-[7/span_6] content-stretch flex h-[621.11px] items-center justify-center justify-self-stretch relative row-1 shrink-0" data-name="Background">
      <Container149 />
    </div>
  );
}

function Container148() {
  return (
    <div className="grid grid-cols-[repeat(12,minmax(0,1fr))] grid-rows-[_621.11px] relative shrink-0 w-full" data-name="Container">
      <ImageProrudct />
      <Background5 />
    </div>
  );
}

function Container147() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative self-stretch" data-name="Container">
      <Container148 />
    </div>
  );
}

function Section8() {
  return (
    <div className="content-stretch flex h-[621.112px] items-start justify-center relative shrink-0 w-full" data-name="Section">
      <Container147 />
    </div>
  );
}

function Container168() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0" data-name="Container">
      <div className="flex flex-col font-['Oswald:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#222] text-[30px] uppercase whitespace-nowrap">
        <p className="leading-[45px]">LATEST NEWS</p>
      </div>
    </div>
  );
}

function Svg() {
  return (
    <div className="relative shrink-0 size-[20px]" data-name="SVG">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 20 20">
        <g id="SVG">
          <path d={svgPaths.p3c5ae600} fill="var(--fill-0, black)" id="Vector" />
        </g>
      </svg>
    </div>
  );
}

function Background6() {
  return (
    <div className="bg-[#f2f2f2] content-stretch flex items-center justify-center relative rounded-[9999px] shrink-0 size-[48px]" data-name="Background">
      <Svg />
    </div>
  );
}

function Svg1() {
  return (
    <div className="relative shrink-0 size-[20px]" data-name="SVG">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 20 20">
        <g id="SVG">
          <path d={svgPaths.p13b5d880} fill="var(--fill-0, black)" id="Vector" />
        </g>
      </svg>
    </div>
  );
}

function Background7() {
  return (
    <div className="bg-[#f2f2f2] content-stretch flex items-center justify-center relative rounded-[9999px] shrink-0 size-[48px]" data-name="Background">
      <Svg1 />
    </div>
  );
}

function Container169() {
  return (
    <div className="content-stretch flex gap-[10px] items-center justify-center relative shrink-0" data-name="Container">
      <Background6 />
      <Background7 />
    </div>
  );
}

function Container167() {
  return (
    <div className="content-stretch flex flex-[1_0_0] items-center justify-between min-w-px relative" data-name="Container">
      <Container168 />
      <Container169 />
    </div>
  );
}

function Container166() {
  return (
    <div className="absolute content-stretch flex items-center justify-center left-0 right-0 top-0" data-name="Container">
      <Container167 />
    </div>
  );
}

function Container174() {
  return <div className="flex-[1_0_0] min-w-px relative self-stretch" data-name="Container" />;
}

function Container173() {
  return (
    <div className="content-stretch flex h-[648.2px] items-start justify-center relative shrink-0 w-full" data-name="Container">
      <Container174 />
    </div>
  );
}

function Container172() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px relative self-stretch shrink-0 w-[440px]" data-name="Container">
      <Container173 />
    </div>
  );
}

function Container177() {
  return <div className="flex-[1_0_0] min-w-px relative self-stretch" data-name="Container" />;
}

function Container176() {
  return (
    <div className="content-stretch flex h-[648.2px] items-start justify-center relative shrink-0 w-full" data-name="Container">
      <Container177 />
    </div>
  );
}

function Container175() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px relative self-stretch shrink-0 w-[440px]" data-name="Container">
      <Container176 />
    </div>
  );
}

function Container180() {
  return <div className="flex-[1_0_0] min-w-px relative self-stretch" data-name="Container" />;
}

function Container179() {
  return (
    <div className="content-stretch flex h-[648.2px] items-start justify-center relative shrink-0 w-full" data-name="Container">
      <Container180 />
    </div>
  );
}

function Container178() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px relative self-stretch shrink-0 w-[440px]" data-name="Container">
      <Container179 />
    </div>
  );
}

function Container183() {
  return <div className="flex-[1_0_0] min-w-px relative self-stretch" data-name="Container" />;
}

function Container182() {
  return (
    <div className="content-stretch flex h-[648.2px] items-start justify-center relative shrink-0 w-full" data-name="Container">
      <Container183 />
    </div>
  );
}

function Container181() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px relative self-stretch shrink-0 w-[440px]" data-name="Container">
      <Container182 />
    </div>
  );
}

function Container186() {
  return <div className="flex-[1_0_0] min-w-px relative self-stretch" data-name="Container" />;
}

function Container185() {
  return (
    <div className="content-stretch flex h-[648.2px] items-start justify-center relative shrink-0 w-full" data-name="Container">
      <Container186 />
    </div>
  );
}

function Container184() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px relative self-stretch shrink-0 w-[440px]" data-name="Container">
      <Container185 />
    </div>
  );
}

function Container189() {
  return <div className="flex-[1_0_0] min-w-px relative self-stretch" data-name="Container" />;
}

function Container188() {
  return (
    <div className="content-stretch flex h-[648.2px] items-start justify-center relative shrink-0 w-full" data-name="Container">
      <Container189 />
    </div>
  );
}

function Container187() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px relative self-stretch shrink-0 w-[440px]" data-name="Container">
      <Container188 />
    </div>
  );
}

function Link5SweetOutfitIdeasYoullWearWellBeyondValentinesDay() {
  return (
    <div className="aspect-[410/390] min-h-[390px] relative shrink-0 w-full" data-name="Link → 5 Sweet Outfit Ideas You’ll Wear Well Beyond Valentine’s Day">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute h-full left-[-23.47%] max-w-none top-0 w-[146.93%]" src={imgLink5SweetOutfitIdeasYoullWearWellBeyondValentinesDay} />
      </div>
    </div>
  );
}

function Container196() {
  return (
    <div className="h-[7.781px] relative shrink-0 w-[15.781px]" data-name="Container">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15.7812 7.78067">
        <g id="Container">
          <path d={svgPaths.p3b50080} fill="var(--fill-0, #A9A9A9)" id="Symbol" />
        </g>
      </svg>
    </div>
  );
}

function Container197() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0" data-name="Container">
      <div className="flex flex-col font-['Jost:SemiBold',sans-serif] font-semibold justify-center leading-[0] relative shrink-0 text-[#a9a9a9] text-[10px] uppercase whitespace-nowrap">
        <p className="leading-[24px]">24TH DEC 2025</p>
      </div>
    </div>
  );
}

function Container195() {
  return (
    <div className="content-stretch flex items-center pr-[22px] relative shrink-0" data-name="Container">
      <Container196 />
      <Container197 />
    </div>
  );
}

function Container199() {
  return (
    <div className="h-[10px] relative shrink-0 w-[18px]" data-name="Container">
      <div className="absolute inset-[-3.95%_0_0_0]">
        <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 18 10.3949">
          <g id="Container">
            <path d={svgPaths.p127b7a00} fill="var(--fill-0, #A9A9A9)" id="Symbol" />
          </g>
        </svg>
      </div>
    </div>
  );
}

function Container200() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0" data-name="Container">
      <div className="flex flex-col font-['Jost:SemiBold',sans-serif] font-semibold justify-center leading-[0] relative shrink-0 text-[#a9a9a9] text-[10px] uppercase whitespace-nowrap">
        <p className="leading-[24px]">ADMIN</p>
      </div>
    </div>
  );
}

function Container198() {
  return (
    <div className="content-stretch flex items-center relative shrink-0" data-name="Container">
      <Container199 />
      <Container200 />
    </div>
  );
}

function Container194() {
  return (
    <div className="content-stretch flex items-center relative shrink-0 w-full" data-name="Container">
      <Container195 />
      <Container198 />
    </div>
  );
}

function Container201() {
  return (
    <div className="relative shrink-0 w-full" data-name="Container">
      <div className="overflow-clip rounded-[inherit] size-full">
        <div className="content-stretch flex items-start pr-[67.29px] relative size-full">
          <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#222] text-[16px] whitespace-nowrap">
            <p className="leading-[24px] mb-0">5 Sweet Outfit Ideas You’ll Wear Well Beyond</p>
            <p className="leading-[24px]">Valentine’s Day</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function Link38() {
  return (
    <div className="content-stretch flex flex-col items-start pb-[7.2px] relative shrink-0 w-full" data-name="Link">
      <Container201 />
    </div>
  );
}

function Container202() {
  return (
    <div className="content-stretch flex flex-col items-start overflow-clip pt-[4px] relative shrink-0 w-full" data-name="Container">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[#505157] text-[14px] w-full">
        <p className="leading-[21px] mb-0">Duis commodo faucibus lectus, et accumsan quam egestas at.</p>
        <p className="leading-[21px] mb-0">Praesent eros mi, condimentum sit amet felis quis, hendrerit</p>
        <p className="leading-[21px]">ullamcorper turpis vehicula nibh...</p>
      </div>
    </div>
  );
}

function Margin10() {
  return (
    <div className="content-stretch flex flex-col items-start pr-[20px] relative shrink-0 z-[2]" data-name="Margin">
      <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#222] text-[14px] uppercase whitespace-nowrap">
        <p className="leading-[24px]">READ MORE</p>
      </div>
    </div>
  );
}

function Container203() {
  return (
    <div className="h-[6.866px] relative shrink-0 w-[11.057px] z-[1]" data-name="Container">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.0566 6.86602">
        <g id="Container">
          <path d={svgPaths.p106f9680} fill="var(--fill-0, #222222)" id="Symbol" />
        </g>
      </svg>
    </div>
  );
}

function Link39() {
  return (
    <div className="relative shrink-0 w-full" data-name="Link">
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex gap-[0.01px] isolate items-center pr-[28px] pt-[16px] relative size-full">
          <Margin10 />
          <Container203 />
        </div>
      </div>
    </div>
  );
}

function Container193() {
  return (
    <div className="content-stretch flex flex-col gap-[16px] items-start relative shrink-0 w-full" data-name="Container">
      <Container194 />
      <Link38 />
      <Container202 />
      <Link39 />
    </div>
  );
}

function Background8() {
  return (
    <div className="bg-white content-stretch flex flex-col gap-[20px] items-start overflow-clip relative shrink-0 w-full" data-name="Background">
      <Link5SweetOutfitIdeasYoullWearWellBeyondValentinesDay />
      <Container193 />
    </div>
  );
}

function Container192() {
  return (
    <div className="flex-[1_0_0] min-w-px relative self-stretch" data-name="Container">
      <div className="content-stretch flex flex-col items-start px-[15px] relative size-full">
        <Background8 />
      </div>
    </div>
  );
}

function Container191() {
  return (
    <div className="content-stretch flex h-[644.2px] items-start justify-center relative shrink-0 w-full" data-name="Container">
      <Container192 />
    </div>
  );
}

function Container190() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px relative self-stretch shrink-0 w-[440px]" data-name="Container">
      <Container191 />
    </div>
  );
}

function Link12MustHaveShoesEveryFashionLoverNeedsInTheirCloset() {
  return (
    <div className="aspect-[410/390] min-h-[390px] relative shrink-0 w-full" data-name="Link → 12 Must-Have Shoes Every Fashion Lover Needs in Their Closet">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute h-full left-[-23.47%] max-w-none top-0 w-[146.93%]" src={imgLink12MustHaveShoesEveryFashionLoverNeedsInTheirCloset} />
      </div>
    </div>
  );
}

function Container210() {
  return (
    <div className="h-[7.781px] relative shrink-0 w-[15.781px]" data-name="Container">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15.7812 7.78067">
        <g id="Container">
          <path d={svgPaths.p3b50080} fill="var(--fill-0, #A9A9A9)" id="Symbol" />
        </g>
      </svg>
    </div>
  );
}

function Container211() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0" data-name="Container">
      <div className="flex flex-col font-['Jost:SemiBold',sans-serif] font-semibold justify-center leading-[0] relative shrink-0 text-[#a9a9a9] text-[10px] uppercase whitespace-nowrap">
        <p className="leading-[24px]">24TH DEC 2025</p>
      </div>
    </div>
  );
}

function Container209() {
  return (
    <div className="content-stretch flex items-center pr-[22px] relative shrink-0" data-name="Container">
      <Container210 />
      <Container211 />
    </div>
  );
}

function Container213() {
  return (
    <div className="h-[10px] relative shrink-0 w-[18px]" data-name="Container">
      <div className="absolute inset-[-3.95%_0_0_0]">
        <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 18 10.3949">
          <g id="Container">
            <path d={svgPaths.p127b7a00} fill="var(--fill-0, #A9A9A9)" id="Symbol" />
          </g>
        </svg>
      </div>
    </div>
  );
}

function Container214() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0" data-name="Container">
      <div className="flex flex-col font-['Jost:SemiBold',sans-serif] font-semibold justify-center leading-[0] relative shrink-0 text-[#a9a9a9] text-[10px] uppercase whitespace-nowrap">
        <p className="leading-[24px]">ADMIN</p>
      </div>
    </div>
  );
}

function Container212() {
  return (
    <div className="content-stretch flex items-center relative shrink-0" data-name="Container">
      <Container213 />
      <Container214 />
    </div>
  );
}

function Container208() {
  return (
    <div className="content-stretch flex items-center relative shrink-0 w-full" data-name="Container">
      <Container209 />
      <Container212 />
    </div>
  );
}

function Container215() {
  return (
    <div className="relative shrink-0 w-full" data-name="Container">
      <div className="overflow-clip rounded-[inherit] size-full">
        <div className="content-stretch flex items-start pr-[32.61px] relative size-full">
          <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#222] text-[16px] whitespace-nowrap">
            <p className="leading-[24px] mb-0">12 Must-Have Shoes Every Fashion Lover Needs in</p>
            <p className="leading-[24px]">Their Closet</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function Link40() {
  return (
    <div className="content-stretch flex flex-col items-start pb-[7.2px] relative shrink-0 w-full" data-name="Link">
      <Container215 />
    </div>
  );
}

function Container216() {
  return (
    <div className="content-stretch flex flex-col items-start overflow-clip pt-[4px] relative shrink-0 w-full" data-name="Container">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[#505157] text-[14px] w-full">
        <p className="leading-[21px] mb-0">Duis commodo faucibus lectus, et accumsan quam egestas at.</p>
        <p className="leading-[21px] mb-0">Praesent eros mi, condimentum sit amet felis quis, hendrerit</p>
        <p className="leading-[21px]">ullamcorper turpis vehicula nibh...</p>
      </div>
    </div>
  );
}

function Margin11() {
  return (
    <div className="content-stretch flex flex-col items-start pr-[20px] relative shrink-0 z-[2]" data-name="Margin">
      <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#222] text-[14px] uppercase whitespace-nowrap">
        <p className="leading-[24px]">READ MORE</p>
      </div>
    </div>
  );
}

function Container217() {
  return (
    <div className="h-[6.866px] relative shrink-0 w-[11.057px] z-[1]" data-name="Container">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.0566 6.86602">
        <g id="Container">
          <path d={svgPaths.p106f9680} fill="var(--fill-0, #222222)" id="Symbol" />
        </g>
      </svg>
    </div>
  );
}

function Link41() {
  return (
    <div className="relative shrink-0 w-full" data-name="Link">
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex gap-[0.01px] isolate items-center pr-[28px] pt-[16px] relative size-full">
          <Margin11 />
          <Container217 />
        </div>
      </div>
    </div>
  );
}

function Container207() {
  return (
    <div className="content-stretch flex flex-col gap-[16px] items-start relative shrink-0 w-full" data-name="Container">
      <Container208 />
      <Link40 />
      <Container216 />
      <Link41 />
    </div>
  );
}

function Background9() {
  return (
    <div className="bg-white content-stretch flex flex-col gap-[20px] items-start overflow-clip relative shrink-0 w-full" data-name="Background">
      <Link12MustHaveShoesEveryFashionLoverNeedsInTheirCloset />
      <Container207 />
    </div>
  );
}

function Container206() {
  return (
    <div className="flex-[1_0_0] min-w-px relative self-stretch" data-name="Container">
      <div className="content-stretch flex flex-col items-start px-[15px] relative size-full">
        <Background9 />
      </div>
    </div>
  );
}

function Container205() {
  return (
    <div className="content-stretch flex h-[644.2px] items-start justify-center relative shrink-0 w-full" data-name="Container">
      <Container206 />
    </div>
  );
}

function Container204() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px relative self-stretch shrink-0 w-[440px]" data-name="Container">
      <Container205 />
    </div>
  );
}

function LinkThisScandiInfluencersShoeCollaborationIsTheGiftThatKeepsOnGiving() {
  return (
    <div className="aspect-[410/390] min-h-[390px] relative shrink-0 w-full" data-name="Link → This Scandi Influencer’s Shoe Collaboration Is The Gift That Keeps On Giving">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute h-full left-[-23.47%] max-w-none top-0 w-[146.93%]" src={imgLinkThisScandiInfluencersShoeCollaborationIsTheGiftThatKeepsOnGiving} />
      </div>
    </div>
  );
}

function Container224() {
  return (
    <div className="h-[7.781px] relative shrink-0 w-[15.781px]" data-name="Container">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15.7812 7.78067">
        <g id="Container">
          <path d={svgPaths.p3b50080} fill="var(--fill-0, #A9A9A9)" id="Symbol" />
        </g>
      </svg>
    </div>
  );
}

function Container225() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0" data-name="Container">
      <div className="flex flex-col font-['Jost:SemiBold',sans-serif] font-semibold justify-center leading-[0] relative shrink-0 text-[#a9a9a9] text-[10px] uppercase whitespace-nowrap">
        <p className="leading-[24px]">24TH DEC 2025</p>
      </div>
    </div>
  );
}

function Container223() {
  return (
    <div className="content-stretch flex items-center pr-[22px] relative shrink-0" data-name="Container">
      <Container224 />
      <Container225 />
    </div>
  );
}

function Container227() {
  return (
    <div className="h-[10px] relative shrink-0 w-[18px]" data-name="Container">
      <div className="absolute inset-[-3.95%_0_0_0]">
        <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 18 10.3949">
          <g id="Container">
            <path d={svgPaths.p127b7a00} fill="var(--fill-0, #A9A9A9)" id="Symbol" />
          </g>
        </svg>
      </div>
    </div>
  );
}

function Container228() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0" data-name="Container">
      <div className="flex flex-col font-['Jost:SemiBold',sans-serif] font-semibold justify-center leading-[0] relative shrink-0 text-[#a9a9a9] text-[10px] uppercase whitespace-nowrap">
        <p className="leading-[24px]">ADMIN</p>
      </div>
    </div>
  );
}

function Container226() {
  return (
    <div className="content-stretch flex items-center relative shrink-0" data-name="Container">
      <Container227 />
      <Container228 />
    </div>
  );
}

function Container222() {
  return (
    <div className="content-stretch flex items-center relative shrink-0 w-full" data-name="Container">
      <Container223 />
      <Container226 />
    </div>
  );
}

function Container229() {
  return (
    <div className="relative shrink-0 w-full" data-name="Container">
      <div className="overflow-clip rounded-[inherit] size-full">
        <div className="content-stretch flex items-start pr-[4.25px] relative size-full">
          <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#222] text-[16px] whitespace-nowrap">
            <p className="leading-[24px] mb-0">This Scandi Influencer’s Shoe Collaboration Is The Gift</p>
            <p className="leading-[24px]">That Keeps On Giving</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function Link42() {
  return (
    <div className="content-stretch flex flex-col items-start pb-[7.2px] relative shrink-0 w-full" data-name="Link">
      <Container229 />
    </div>
  );
}

function Container230() {
  return (
    <div className="content-stretch flex flex-col items-start overflow-clip pt-[4px] relative shrink-0 w-full" data-name="Container">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[#505157] text-[14px] w-full">
        <p className="leading-[21px] mb-0">Duis commodo faucibus lectus, et accumsan quam egestas at.</p>
        <p className="leading-[21px] mb-0">Praesent eros mi, condimentum sit amet felis quis, hendrerit</p>
        <p className="leading-[21px]">ullamcorper turpis vehicula nibh...</p>
      </div>
    </div>
  );
}

function Margin12() {
  return (
    <div className="content-stretch flex flex-col items-start pr-[20px] relative shrink-0 z-[2]" data-name="Margin">
      <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#222] text-[14px] uppercase whitespace-nowrap">
        <p className="leading-[24px]">READ MORE</p>
      </div>
    </div>
  );
}

function Container231() {
  return (
    <div className="h-[6.866px] relative shrink-0 w-[11.057px] z-[1]" data-name="Container">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.0566 6.86602">
        <g id="Container">
          <path d={svgPaths.p106f9680} fill="var(--fill-0, #222222)" id="Symbol" />
        </g>
      </svg>
    </div>
  );
}

function Link43() {
  return (
    <div className="relative shrink-0 w-full" data-name="Link">
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex gap-[0.01px] isolate items-center pr-[28px] pt-[16px] relative size-full">
          <Margin12 />
          <Container231 />
        </div>
      </div>
    </div>
  );
}

function Container221() {
  return (
    <div className="content-stretch flex flex-col gap-[16px] items-start relative shrink-0 w-full" data-name="Container">
      <Container222 />
      <Link42 />
      <Container230 />
      <Link43 />
    </div>
  );
}

function Background10() {
  return (
    <div className="bg-white content-stretch flex flex-col gap-[20px] items-start overflow-clip relative shrink-0 w-full" data-name="Background">
      <LinkThisScandiInfluencersShoeCollaborationIsTheGiftThatKeepsOnGiving />
      <Container221 />
    </div>
  );
}

function Container220() {
  return (
    <div className="flex-[1_0_0] min-w-px relative self-stretch" data-name="Container">
      <div className="content-stretch flex flex-col items-start px-[15px] relative size-full">
        <Background10 />
      </div>
    </div>
  );
}

function Container219() {
  return (
    <div className="content-stretch flex h-[644.2px] items-start justify-center relative shrink-0 w-full" data-name="Container">
      <Container220 />
    </div>
  );
}

function Container218() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px relative self-stretch shrink-0 w-[440px]" data-name="Container">
      <Container219 />
    </div>
  );
}

function LinkHowToBeGoodWithMoneyAccordingTo4FemaleFinancialExperts() {
  return (
    <div className="aspect-[410/390] min-h-[390px] relative shrink-0 w-full" data-name="Link → How To Be “Good With Money” According To 4 Female Financial Experts">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute h-full left-[-23.47%] max-w-none top-0 w-[146.93%]" src={imgLinkHowToBeGoodWithMoneyAccordingTo4FemaleFinancialExperts} />
      </div>
    </div>
  );
}

function Container238() {
  return (
    <div className="h-[7.781px] relative shrink-0 w-[15.781px]" data-name="Container">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15.7812 7.78067">
        <g id="Container">
          <path d={svgPaths.p3b50080} fill="var(--fill-0, #A9A9A9)" id="Symbol" />
        </g>
      </svg>
    </div>
  );
}

function Container239() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0" data-name="Container">
      <div className="flex flex-col font-['Jost:SemiBold',sans-serif] font-semibold justify-center leading-[0] relative shrink-0 text-[#a9a9a9] text-[10px] uppercase whitespace-nowrap">
        <p className="leading-[24px]">24TH DEC 2025</p>
      </div>
    </div>
  );
}

function Container237() {
  return (
    <div className="content-stretch flex items-center pr-[22px] relative shrink-0" data-name="Container">
      <Container238 />
      <Container239 />
    </div>
  );
}

function Container241() {
  return <div className="h-[10px] relative shrink-0 w-[18px]" data-name="Container" />;
}

function Container240() {
  return (
    <div className="content-stretch flex items-center relative shrink-0" data-name="Container">
      <Container241 />
      <div className="h-[24px] relative shrink-0 w-[33.78px]" data-name="Rectangle" />
    </div>
  );
}

function Container236() {
  return (
    <div className="content-stretch flex items-center relative shrink-0 w-full" data-name="Container">
      <Container237 />
      <Container240 />
    </div>
  );
}

function Container242() {
  return (
    <div className="relative shrink-0 w-full" data-name="Container">
      <div className="overflow-clip rounded-[inherit] size-full">
        <div className="content-stretch flex items-start pr-[51.12px] relative size-full">
          <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#222] text-[16px] whitespace-nowrap">
            <p className="leading-[24px] mb-0">How To Be “Good With Money” According To 4</p>
            <p className="leading-[24px]">Female Financial Experts</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function Link44() {
  return (
    <div className="content-stretch flex flex-col items-start pb-[7.2px] relative shrink-0 w-full" data-name="Link">
      <Container242 />
    </div>
  );
}

function Container243() {
  return (
    <div className="content-stretch flex flex-col items-start overflow-clip pt-[4px] relative shrink-0 w-full" data-name="Container">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[#505157] text-[14px] w-full">
        <p className="leading-[21px] mb-0">Duis commodo faucibus lectus, et accumsan quam egestas at.</p>
        <p className="leading-[21px] mb-0">Praesent eros mi, condimentum sit amet felis quis, hendrerit</p>
        <p className="leading-[21px]">ullamcorper turpis vehicula nibh...</p>
      </div>
    </div>
  );
}

function Margin13() {
  return (
    <div className="content-stretch flex flex-col items-start pr-[20px] relative shrink-0 z-[2]" data-name="Margin">
      <div className="flex flex-col font-['Jost:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#222] text-[14px] uppercase whitespace-nowrap">
        <p className="leading-[24px]">READ MORE</p>
      </div>
    </div>
  );
}

function Container244() {
  return <div className="h-[28px] relative shrink-0 w-[12.74px] z-[1]" data-name="Container" />;
}

function Link45() {
  return (
    <div className="relative shrink-0 w-full" data-name="Link">
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex gap-[0.01px] isolate items-center pr-[28px] pt-[16px] relative size-full">
          <Margin13 />
          <Container244 />
        </div>
      </div>
    </div>
  );
}

function Container235() {
  return (
    <div className="content-stretch flex flex-col gap-[16px] items-start relative shrink-0 w-full" data-name="Container">
      <Container236 />
      <Link44 />
      <Container243 />
      <Link45 />
    </div>
  );
}

function Background11() {
  return (
    <div className="bg-white content-stretch flex flex-col gap-[20px] items-start overflow-clip relative shrink-0 w-full" data-name="Background">
      <LinkHowToBeGoodWithMoneyAccordingTo4FemaleFinancialExperts />
      <Container235 />
    </div>
  );
}

function Container234() {
  return (
    <div className="flex-[1_0_0] min-w-px relative self-stretch" data-name="Container">
      <div className="content-stretch flex flex-col items-start px-[15px] relative size-full">
        <Background11 />
      </div>
    </div>
  );
}

function Container233() {
  return (
    <div className="content-stretch flex h-[648.2px] items-start justify-center relative shrink-0 w-full" data-name="Container">
      <Container234 />
    </div>
  );
}

function Container232() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px relative self-stretch shrink-0 w-[440px]" data-name="Container">
      <Container233 />
    </div>
  );
}

function Container247() {
  return <div className="flex-[1_0_0] min-w-px relative self-stretch" data-name="Container" />;
}

function Container246() {
  return (
    <div className="content-stretch flex h-[648.2px] items-start justify-center relative shrink-0 w-full" data-name="Container">
      <Container247 />
    </div>
  );
}

function Container245() {
  return (
    <div className="content-stretch flex flex-col items-start min-h-px relative self-stretch shrink-0 w-[440px]" data-name="Container">
      <Container246 />
    </div>
  );
}

function Container171() {
  return (
    <div className="absolute content-stretch flex h-[648.2px] items-start left-[-2640px] top-0 w-[4840px]" data-name="Container">
      <Container172 />
      <Container175 />
      <Container178 />
      <Container181 />
      <Container184 />
      <Container187 />
      <Container190 />
      <Container204 />
      <Container218 />
      <Container232 />
      <Container245 />
    </div>
  );
}

function Container170() {
  return (
    <div className="absolute h-[648.2px] left-[-15px] overflow-clip right-[-783px] top-[98px]" data-name="Container">
      <Container171 />
    </div>
  );
}

function Container165() {
  return (
    <div className="h-[746.2px] relative shrink-0 w-full" data-name="Container">
      <Container166 />
      <Container170 />
    </div>
  );
}

function Container164() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative self-stretch" data-name="Container">
      <Container165 />
    </div>
  );
}

function Container163() {
  return (
    <div className="content-stretch flex h-[746.2px] items-start justify-center max-w-[1290px] relative shrink-0 w-full" data-name="Container">
      <Container164 />
    </div>
  );
}

function Section9() {
  return (
    <div className="relative shrink-0 w-full" data-name="Section">
      <div className="overflow-clip rounded-[inherit] size-full">
        <div className="content-stretch flex flex-col items-start px-[65.4px] py-[140px] relative size-full">
          <Container163 />
        </div>
      </div>
    </div>
  );
}

function PolicyReturnSvg() {
  return (
    <div className="relative shrink-0 size-[46px]" data-name="policy-return.svg">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 46 46">
        <g id="policy-return.svg">
          <path d={svgPaths.p2ee06880} fill="var(--fill-0, black)" id="Vector" />
          <path d={svgPaths.p203f3800} fill="var(--fill-0, black)" id="Vector_2" />
          <path d={svgPaths.p841c780} fill="var(--fill-0, black)" id="Vector_3" />
          <path d={svgPaths.p2d4de980} fill="var(--fill-0, black)" id="Vector_4" />
        </g>
      </svg>
    </div>
  );
}

function FigurePolicyReturnSvg() {
  return (
    <div className="aspect-[46/46] content-stretch flex flex-col items-center justify-center max-w-[409.989990234375px] overflow-clip relative shrink-0" data-name="Figure → policy-return.svg">
      <PolicyReturnSvg />
    </div>
  );
}

function Heading2() {
  return (
    <div className="content-stretch flex flex-col items-center relative shrink-0 w-full" data-name="Heading 3">
      <div className="flex flex-col font-['Oswald:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#222] text-[18px] text-center uppercase whitespace-nowrap">
        <p className="leading-[36px]">30 DAYS RETURN</p>
      </div>
    </div>
  );
}

function Container254() {
  return (
    <div className="content-stretch flex flex-col items-center relative shrink-0 w-full" data-name="Container">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[#505157] text-[16px] text-center whitespace-nowrap">
        <p className="leading-[24px]">Simply return it within 30 days for an exchange.</p>
      </div>
    </div>
  );
}

function Container253() {
  return (
    <div className="content-stretch flex flex-col gap-[8px] items-start relative shrink-0 w-full" data-name="Container">
      <Heading2 />
      <Container254 />
    </div>
  );
}

function Container252() {
  return (
    <div className="content-stretch flex flex-col gap-[19.2px] items-center relative shrink-0 w-full" data-name="Container">
      <FigurePolicyReturnSvg />
      <Container253 />
    </div>
  );
}

function Container251() {
  return (
    <div className="content-stretch flex flex-col h-full items-start relative shrink-0 w-[409.99px]" data-name="Container">
      <Container252 />
    </div>
  );
}

function Container250() {
  return (
    <div className="flex-[1_0_0] min-h-px min-w-px relative self-stretch" data-name="Container">
      <div className="flex flex-row justify-center min-h-[inherit] size-full">
        <div className="content-stretch flex items-start justify-center min-h-[inherit] py-[15px] relative size-full">
          <Container251 />
        </div>
      </div>
    </div>
  );
}

function PolicyFreedeliverySvg() {
  return (
    <div className="relative shrink-0 size-[46px]" data-name="policy-freedelivery.svg">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 46 46">
        <g id="policy-freedelivery.svg">
          <path d={svgPaths.p1e42d780} fill="var(--fill-0, black)" id="Vector" />
        </g>
      </svg>
    </div>
  );
}

function FigurePolicyFreedeliverySvg() {
  return (
    <div className="aspect-[46/46] content-stretch flex flex-col items-center justify-center max-w-[409.989990234375px] overflow-clip relative shrink-0" data-name="Figure → policy-freedelivery.svg">
      <PolicyFreedeliverySvg />
    </div>
  );
}

function Heading3() {
  return (
    <div className="content-stretch flex flex-col items-center relative shrink-0 w-full" data-name="Heading 3">
      <div className="flex flex-col font-['Oswald:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#222] text-[18px] text-center uppercase whitespace-nowrap">
        <p className="leading-[36px]">FREE US DELIVERY</p>
      </div>
    </div>
  );
}

function Container259() {
  return (
    <div className="content-stretch flex flex-col items-center relative shrink-0 w-full" data-name="Container">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[#505157] text-[16px] text-center whitespace-nowrap">
        <p className="leading-[24px]">On orders over $200!</p>
      </div>
    </div>
  );
}

function Container258() {
  return (
    <div className="content-stretch flex flex-col gap-[8px] items-start relative shrink-0 w-full" data-name="Container">
      <Heading3 />
      <Container259 />
    </div>
  );
}

function Container257() {
  return (
    <div className="content-stretch flex flex-col gap-[19.2px] items-center relative shrink-0 w-full" data-name="Container">
      <FigurePolicyFreedeliverySvg />
      <Container258 />
    </div>
  );
}

function Container256() {
  return (
    <div className="content-stretch flex flex-col h-full items-start relative shrink-0 w-[409.99px]" data-name="Container">
      <Container257 />
    </div>
  );
}

function Container255() {
  return (
    <div className="flex-[1_0_0] min-h-px min-w-px relative self-stretch" data-name="Container">
      <div className="flex flex-row justify-center min-h-[inherit] size-full">
        <div className="content-stretch flex items-start justify-center min-h-[inherit] py-[15px] relative size-full">
          <Container256 />
        </div>
      </div>
    </div>
  );
}

function PolicySupportSvg() {
  return (
    <div className="relative shrink-0 size-[32px]" data-name="policy-support.svg">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 32 32">
        <g clipPath="url(#clip0_1_5666)" id="policy-support.svg">
          <path d={svgPaths.p29282200} fill="var(--fill-0, black)" id="Vector" />
        </g>
        <defs>
          <clipPath id="clip0_1_5666">
            <rect fill="white" height="32" width="32" />
          </clipPath>
        </defs>
      </svg>
    </div>
  );
}

function FigurePolicySupportSvg() {
  return (
    <div className="absolute content-stretch flex flex-col items-center justify-center left-[181.98px] max-w-[409.989990234375px] overflow-clip top-0 w-[32px]" data-name="Figure → policy-support.svg">
      <PolicySupportSvg />
    </div>
  );
}

function Heading4() {
  return (
    <div className="content-stretch flex flex-col items-center relative shrink-0 w-full" data-name="Heading 3">
      <div className="flex flex-col font-['Oswald:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#222] text-[18px] text-center uppercase whitespace-nowrap">
        <p className="leading-[36px]">SUPPORT 24/7</p>
      </div>
    </div>
  );
}

function Container264() {
  return (
    <div className="content-stretch flex flex-col items-center relative shrink-0 w-full" data-name="Container">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[#505157] text-[16px] text-center whitespace-nowrap">
        <p className="leading-[24px]">Contact us 24 hours a day, 7 days a week</p>
      </div>
    </div>
  );
}

function Container263() {
  return (
    <div className="absolute content-stretch flex flex-col gap-[8px] items-start left-0 right-0 top-[65.2px]" data-name="Container">
      <Heading4 />
      <Container264 />
    </div>
  );
}

function Container262() {
  return (
    <div className="h-[133.2px] relative shrink-0 w-full" data-name="Container">
      <FigurePolicySupportSvg />
      <Container263 />
    </div>
  );
}

function Container261() {
  return (
    <div className="content-stretch flex flex-col h-full items-start relative shrink-0 w-[409.99px]" data-name="Container">
      <Container262 />
    </div>
  );
}

function Container260() {
  return (
    <div className="flex-[1_0_0] min-h-px min-w-px relative self-stretch" data-name="Container">
      <div className="flex flex-row justify-center min-h-[inherit] size-full">
        <div className="content-stretch flex items-start justify-center min-h-[inherit] py-[15px] relative size-full">
          <Container261 />
        </div>
      </div>
    </div>
  );
}

function Container249() {
  return (
    <div className="content-stretch flex h-[163.2px] items-start max-w-[1320px] relative shrink-0 w-full" data-name="Container">
      <Container250 />
      <Container255 />
      <Container260 />
    </div>
  );
}

function Section11() {
  return (
    <div className="flex-[1_0_0] min-w-px relative self-stretch" data-name="Section">
      <div className="content-stretch flex flex-col items-start px-[50.4px] relative size-full">
        <Container249 />
      </div>
    </div>
  );
}

function Container248() {
  return (
    <div className="content-stretch flex h-[163.2px] items-start justify-center relative shrink-0 w-full" data-name="Container">
      <Section11 />
    </div>
  );
}

function Section10() {
  return (
    <div className="bg-[#f2f2f2] content-stretch flex flex-col items-start py-[80px] relative shrink-0 w-full" data-name="Section">
      <Container248 />
    </div>
  );
}

function Masthead() {
  return (
    <div className="absolute content-stretch flex flex-col items-center left-0 right-[0.2px] top-[80px]" data-name="masthead">
      <Section />
      <Section1 />
      <Section3 />
      <Section5 />
      <Section6 />
      <Section7 />
      <Section8 />
      <Section9 />
      <Section10 />
    </div>
  );
}

function Heading5() {
  return (
    <div className="content-stretch flex flex-col items-start pb-[0.7px] relative shrink-0 w-full" data-name="Heading 2">
      <div className="flex flex-col font-['Oswald:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[18px] text-white uppercase w-full">
        <p className="leading-[27px]">CONTACT US</p>
      </div>
    </div>
  );
}

function Container270() {
  return (
    <div className="absolute content-stretch flex flex-col inset-[-0.7px_0_20px_0] items-start" data-name="Container">
      <Heading5 />
    </div>
  );
}

function Margin14() {
  return (
    <div className="h-[57px] relative shrink-0 w-full" data-name="Margin">
      <Container270 />
    </div>
  );
}

function Container272() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0 w-[389.99px]" data-name="Container">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[16px] text-white whitespace-nowrap">
        <p className="leading-[24px] mb-0">Morbi ullamcorper ligula sit amet efficitur pellentesque.</p>
        <p className="leading-[24px]">Aliquam ornare quam tellus ultricies molestie tortor.</p>
      </div>
    </div>
  );
}

function Container271() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px relative w-full" data-name="Container">
      <Container272 />
    </div>
  );
}

function Margin15() {
  return (
    <div className="content-stretch flex flex-col h-[73px] items-start justify-center pb-[20px] relative shrink-0 w-full" data-name="Margin">
      <Container271 />
    </div>
  );
}

function ImageContact1Png() {
  return (
    <div className="max-w-[24.489999771118164px] relative shrink-0 size-[24.49px]" data-name="image-contact-1.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgImageContact1Png} />
      </div>
    </div>
  );
}

function Figure() {
  return (
    <div className="content-stretch flex flex-col items-start justify-center relative shrink-0 w-[24.49px]" data-name="Figure">
      <ImageContact1Png />
    </div>
  );
}

function FigureMargin() {
  return (
    <div className="content-stretch flex flex-col items-start pr-[27px] relative shrink-0 w-[51.49px]" data-name="Figure:margin">
      <Figure />
    </div>
  );
}

function Paragraph() {
  return (
    <div className="flex-[1_0_0] font-['Jost:Bold',sans-serif] font-bold h-[27px] leading-[0] min-w-px relative text-white uppercase whitespace-nowrap" data-name="Paragraph">
      <div className="-translate-y-1/2 absolute flex flex-col justify-center left-0 text-[13px] top-[15.35px]">
        <p className="leading-[19.5px]">HOTLINE:</p>
      </div>
      <div className="-translate-y-1/2 absolute flex flex-col justify-center left-[64.17px] text-[18px] top-[13.5px]">
        <p className="leading-[27px]">+0123 456 789</p>
      </div>
    </div>
  );
}

function Container274() {
  return (
    <div className="content-stretch flex items-center relative shrink-0 w-full" data-name="Container">
      <FigureMargin />
      <Paragraph />
    </div>
  );
}

function Container273() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px relative w-full" data-name="Container">
      <Container274 />
    </div>
  );
}

function Margin16() {
  return (
    <div className="content-stretch flex flex-col h-[47px] items-start justify-center pb-[20px] relative shrink-0 w-full" data-name="Margin">
      <Container273 />
    </div>
  );
}

function ImageContact2Png() {
  return (
    <div className="h-[18.36px] max-w-[24.489999771118164px] relative shrink-0 w-[24.49px]" data-name="image-contact-2.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute h-[100.04%] left-0 max-w-none top-[-0.02%] w-full" src={imgImageContact2Png} />
      </div>
    </div>
  );
}

function Figure1() {
  return (
    <div className="content-stretch flex flex-col items-start justify-center relative shrink-0 w-[24.49px]" data-name="Figure">
      <ImageContact2Png />
    </div>
  );
}

function FigureMargin1() {
  return (
    <div className="content-stretch flex flex-col items-start pr-[27px] relative shrink-0 w-[51.49px]" data-name="Figure:margin">
      <Figure1 />
    </div>
  );
}

function Container277() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative" data-name="Container">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[16px] text-white w-full">
        <p className="leading-[24px]">info@example.com</p>
      </div>
    </div>
  );
}

function Container276() {
  return (
    <div className="content-stretch flex items-start relative shrink-0 w-full" data-name="Container">
      <FigureMargin1 />
      <Container277 />
    </div>
  );
}

function Container275() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px relative w-full" data-name="Container">
      <Container276 />
    </div>
  );
}

function Margin17() {
  return (
    <div className="content-stretch flex flex-col h-[44px] items-start justify-center pb-[20px] relative shrink-0 w-full" data-name="Margin">
      <Container275 />
    </div>
  );
}

function FigureImageContact3Png() {
  return (
    <div className="max-w-[24.489999771118164px] relative shrink-0 size-[24.49px]" data-name="Figure → image-contact-3.png">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute left-0 max-w-none size-full top-0" src={imgFigureImageContact3Png} />
      </div>
    </div>
  );
}

function FigureMargin2() {
  return (
    <div className="content-stretch flex flex-col items-start pr-[27px] relative shrink-0 w-[51.49px]" data-name="Figure:margin">
      <FigureImageContact3Png />
    </div>
  );
}

function Container280() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative" data-name="Container">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[16px] text-white w-full">
        <p className="leading-[24px]">Monday till Friday 10 to 6 EST</p>
      </div>
    </div>
  );
}

function Container279() {
  return (
    <div className="content-stretch flex items-start relative shrink-0 w-full" data-name="Container">
      <FigureMargin2 />
      <Container280 />
    </div>
  );
}

function Container278() {
  return (
    <div className="content-stretch flex flex-col h-[24.49px] items-start relative shrink-0 w-full" data-name="Container">
      <Container279 />
    </div>
  );
}

function Container269() {
  return (
    <div className="flex-[1_0_0] h-full min-w-px relative" data-name="Container">
      <div className="flex flex-col justify-center size-full">
        <div className="content-stretch flex flex-col items-start justify-center pb-[17.51px] pt-[15px] px-[15px] relative size-full">
          <Margin14 />
          <Margin15 />
          <Margin16 />
          <Margin17 />
          <Container278 />
        </div>
      </div>
    </div>
  );
}

function Container268() {
  return (
    <div className="content-stretch flex items-start justify-center min-h-px relative self-stretch shrink-0 w-[439.99px]" data-name="Container">
      <Container269 />
    </div>
  );
}

function Heading6() {
  return (
    <div className="content-stretch flex flex-col items-start pb-[0.7px] relative shrink-0 w-full" data-name="Heading 2">
      <div className="flex flex-col font-['Oswald:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[18px] text-white uppercase w-full">
        <p className="leading-[27px]">HELP</p>
      </div>
    </div>
  );
}

function Container283() {
  return (
    <div className="absolute content-stretch flex flex-col inset-[-0.7px_0_20px_0] items-start" data-name="Container">
      <Heading6 />
    </div>
  );
}

function Margin18() {
  return (
    <div className="h-[57px] relative shrink-0 w-full" data-name="Margin">
      <Container283 />
    </div>
  );
}

function Link46() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative" data-name="Link">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-white w-full">
        <p className="leading-[21px]">Help Center</p>
      </div>
    </div>
  );
}

function Item() {
  return (
    <div className="content-stretch flex items-center pb-[6.5px] relative shrink-0 w-full" data-name="Item">
      <Link46 />
    </div>
  );
}

function Link47() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative" data-name="Link">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-white w-full">
        <p className="leading-[21px]">Shipping Info</p>
      </div>
    </div>
  );
}

function Item1() {
  return (
    <div className="content-stretch flex items-center pb-[6.5px] relative shrink-0 w-full" data-name="Item">
      <Link47 />
    </div>
  );
}

function Link48() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative" data-name="Link">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-white w-full">
        <p className="leading-[21px]">Returns</p>
      </div>
    </div>
  );
}

function Item2() {
  return (
    <div className="content-stretch flex items-center pb-[6.5px] relative shrink-0 w-full" data-name="Item">
      <Link48 />
    </div>
  );
}

function Link49() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative" data-name="Link">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-white w-full">
        <p className="leading-[21px]">How To Order</p>
      </div>
    </div>
  );
}

function Item3() {
  return (
    <div className="content-stretch flex items-center pb-[6.5px] relative shrink-0 w-full" data-name="Item">
      <Link49 />
    </div>
  );
}

function Link50() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative" data-name="Link">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-white w-full">
        <p className="leading-[21px]">How To Track</p>
      </div>
    </div>
  );
}

function Item4() {
  return (
    <div className="content-stretch flex items-center pb-[6.5px] relative shrink-0 w-full" data-name="Item">
      <Link50 />
    </div>
  );
}

function Link51() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative" data-name="Link">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-white w-full">
        <p className="leading-[21px]">Size Guide</p>
      </div>
    </div>
  );
}

function Item5() {
  return (
    <div className="content-stretch flex items-center relative shrink-0 w-full" data-name="Item">
      <Link51 />
    </div>
  );
}

function List() {
  return (
    <div className="content-stretch flex flex-col gap-[6.5px] items-start relative shrink-0 w-full" data-name="List">
      <Item />
      <Item1 />
      <Item2 />
      <Item3 />
      <Item4 />
      <Item5 />
    </div>
  );
}

function Container284() {
  return (
    <div className="content-stretch flex flex-col h-[191px] items-start relative shrink-0 w-full" data-name="Container">
      <List />
    </div>
  );
}

function Container282() {
  return (
    <div className="flex-[1_0_0] h-full min-w-px relative" data-name="Container">
      <div className="flex flex-col justify-center size-full">
        <div className="content-stretch flex flex-col items-start justify-center p-[15px] relative size-full">
          <Margin18 />
          <Container284 />
        </div>
      </div>
    </div>
  );
}

function Container281() {
  return (
    <div className="content-stretch flex items-start justify-center min-h-px relative self-stretch shrink-0 w-[220px]" data-name="Container">
      <Container282 />
    </div>
  );
}

function Heading7() {
  return (
    <div className="content-stretch flex flex-col items-start pb-[0.7px] relative shrink-0 w-full" data-name="Heading 2">
      <div className="flex flex-col font-['Oswald:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[18px] text-white uppercase w-full">
        <p className="leading-[27px]">COMPANY</p>
      </div>
    </div>
  );
}

function Container287() {
  return (
    <div className="absolute content-stretch flex flex-col inset-[-0.7px_0_20px_0] items-start" data-name="Container">
      <Heading7 />
    </div>
  );
}

function Margin19() {
  return (
    <div className="h-[57px] relative shrink-0 w-full" data-name="Margin">
      <Container287 />
    </div>
  );
}

function Link52() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative" data-name="Link">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-white w-full">
        <p className="leading-[21px]">About Us</p>
      </div>
    </div>
  );
}

function Item6() {
  return (
    <div className="content-stretch flex items-center pb-[6.5px] relative shrink-0 w-full" data-name="Item">
      <Link52 />
    </div>
  );
}

function Link53() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative" data-name="Link">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-white w-full">
        <p className="leading-[21px]">Our Blog</p>
      </div>
    </div>
  );
}

function Item7() {
  return (
    <div className="content-stretch flex items-center pb-[6.5px] relative shrink-0 w-full" data-name="Item">
      <Link53 />
    </div>
  );
}

function Link54() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative" data-name="Link">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-white w-full">
        <p className="leading-[21px]">Careers</p>
      </div>
    </div>
  );
}

function Item8() {
  return (
    <div className="content-stretch flex items-center pb-[6.5px] relative shrink-0 w-full" data-name="Item">
      <Link54 />
    </div>
  );
}

function Link55() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative" data-name="Link">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-white w-full">
        <p className="leading-[21px]">Store Locations</p>
      </div>
    </div>
  );
}

function Item9() {
  return (
    <div className="content-stretch flex items-center pb-[6.5px] relative shrink-0 w-full" data-name="Item">
      <Link55 />
    </div>
  );
}

function Link56() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative" data-name="Link">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-white w-full">
        <p className="leading-[21px]">Testimonial</p>
      </div>
    </div>
  );
}

function Item10() {
  return (
    <div className="content-stretch flex items-center pb-[6.5px] relative shrink-0 w-full" data-name="Item">
      <Link56 />
    </div>
  );
}

function Link57() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative" data-name="Link">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-white w-full">
        <p className="leading-[21px]">Sitemap</p>
      </div>
    </div>
  );
}

function Item11() {
  return (
    <div className="content-stretch flex items-center relative shrink-0 w-full" data-name="Item">
      <Link57 />
    </div>
  );
}

function List1() {
  return (
    <div className="content-stretch flex flex-col gap-[6.5px] items-start relative shrink-0 w-full" data-name="List">
      <Item6 />
      <Item7 />
      <Item8 />
      <Item9 />
      <Item10 />
      <Item11 />
    </div>
  );
}

function Container288() {
  return (
    <div className="content-stretch flex flex-col h-[191px] items-start relative shrink-0 w-full" data-name="Container">
      <List1 />
    </div>
  );
}

function Container286() {
  return (
    <div className="flex-[1_0_0] h-full min-w-px relative" data-name="Container">
      <div className="flex flex-col justify-center size-full">
        <div className="content-stretch flex flex-col items-start justify-center p-[15px] relative size-full">
          <Margin19 />
          <Container288 />
        </div>
      </div>
    </div>
  );
}

function Container285() {
  return (
    <div className="content-stretch flex items-start justify-center min-h-px relative self-stretch shrink-0 w-[220px]" data-name="Container">
      <Container286 />
    </div>
  );
}

function Heading8() {
  return (
    <div className="content-stretch flex flex-col items-start pb-[0.7px] relative shrink-0 w-full" data-name="Heading 2">
      <div className="flex flex-col font-['Oswald:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[18px] text-white uppercase w-full">
        <p className="leading-[27px]">NEWSLETTER</p>
      </div>
    </div>
  );
}

function Container291() {
  return (
    <div className="absolute content-stretch flex flex-col inset-[-0.7px_0_20px_0] items-start" data-name="Container">
      <Heading8 />
    </div>
  );
}

function Margin20() {
  return (
    <div className="h-[57px] relative shrink-0 w-full" data-name="Margin">
      <Container291 />
    </div>
  );
}

function Container293() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0 w-full" data-name="Container">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-white w-full">
        <p className="leading-[21px] mb-0">Get 15% off your first purchaxse! Plus, be the first to know about</p>
        <p className="leading-[21px]">sales new product launches and exclusive offers!</p>
      </div>
    </div>
  );
}

function Container292() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px relative w-full" data-name="Container">
      <Container293 />
    </div>
  );
}

function Margin21() {
  return (
    <div className="content-stretch flex flex-col h-[74px] items-start justify-center pb-[20px] relative shrink-0 w-full" data-name="Margin">
      <Container292 />
    </div>
  );
}

function Container295() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px overflow-clip relative" data-name="Container">
      <div className="capitalize flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[#959595] text-[13px] w-full">
        <p className="leading-[normal]">Your Email Address</p>
      </div>
    </div>
  );
}

function Input() {
  return (
    <div className="bg-white h-[54px] relative rounded-[54px] shrink-0 w-full" data-name="Input">
      <div className="flex flex-row justify-center overflow-clip rounded-[inherit] size-full">
        <div className="content-stretch flex items-start justify-center pl-[20px] pr-[58px] py-[17.8px] relative size-full">
          <Container295 />
        </div>
      </div>
    </div>
  );
}

function Container296() {
  return (
    <div className="absolute left-[357.97px] size-[50px] top-[2px]" data-name="Container">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 50 50">
        <g id="Container">
          <path d={svgPaths.p9d41f40} fill="var(--fill-0, black)" id="Symbol" />
        </g>
      </svg>
    </div>
  );
}

function FormContactForm() {
  return (
    <div className="content-stretch flex flex-col items-start pb-[10px] relative shrink-0 w-full" data-name="Form - Contact form">
      <Input />
      <Container296 />
    </div>
  );
}

function Container294() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px relative w-full" data-name="Container">
      <FormContactForm />
    </div>
  );
}

function Margin22() {
  return (
    <div className="content-stretch flex flex-col h-[84px] items-start justify-center pb-[20px] relative shrink-0 w-full" data-name="Margin">
      <Container294 />
    </div>
  );
}

function Container299() {
  return (
    <div className="h-[14px] relative shrink-0 w-[8.75px]" data-name="Container">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 8.75 14">
        <g id="Container">
          <path d={svgPaths.p2fb51880} fill="var(--fill-0, white)" id="Symbol" />
        </g>
      </svg>
    </div>
  );
}

function Container298() {
  return (
    <div className="content-stretch flex flex-col items-start pl-[2.62px] pr-[2.63px] relative shrink-0 size-[14px]" data-name="Container">
      <Container299 />
    </div>
  );
}

function ListitemLink() {
  return (
    <div className="content-stretch flex items-center justify-center relative shrink-0 size-[28px]" data-name="Listitem → Link">
      <Container298 />
    </div>
  );
}

function Container301() {
  return (
    <div className="h-[14px] relative shrink-0 w-[12.25px]" data-name="Container">
      <div className="absolute inset-[0_-0.11%]">
        <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 12.2773 14">
          <g id="Container">
            <path d={svgPaths.p26fc3400} fill="var(--fill-0, white)" id="Symbol" />
          </g>
        </svg>
      </div>
    </div>
  );
}

function Container300() {
  return (
    <div className="content-stretch flex flex-col items-start pl-[0.87px] pr-[0.88px] relative shrink-0 size-[14px]" data-name="Container">
      <Container301 />
    </div>
  );
}

function ListitemLink1() {
  return (
    <div className="content-stretch flex items-center justify-center relative shrink-0 size-[28px]" data-name="Listitem → Link">
      <Container300 />
    </div>
  );
}

function Container303() {
  return (
    <div className="h-[14px] relative shrink-0 w-[15.75px]" data-name="Container">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15.75 14">
        <g id="Container">
          <path d={svgPaths.p373030} fill="var(--fill-0, white)" id="Symbol" />
        </g>
      </svg>
    </div>
  );
}

function Container302() {
  return (
    <div className="content-stretch flex flex-col h-[14px] items-start relative shrink-0 w-[14.88px]" data-name="Container">
      <Container303 />
    </div>
  );
}

function ListitemLink2() {
  return (
    <div className="content-stretch flex items-center justify-center pl-[6.12px] pr-[7px] relative shrink-0 size-[28px]" data-name="Listitem → Link">
      <Container302 />
    </div>
  );
}

function Container305() {
  return (
    <div className="h-[14px] relative shrink-0 w-[15.75px]" data-name="Container">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15.75 14">
        <g id="Container">
          <path d={svgPaths.p100feb40} fill="var(--fill-0, white)" id="Symbol" />
        </g>
      </svg>
    </div>
  );
}

function Container304() {
  return (
    <div className="content-stretch flex flex-col h-[14px] items-start relative shrink-0 w-[14.88px]" data-name="Container">
      <Container305 />
    </div>
  );
}

function ListitemLink3() {
  return (
    <div className="content-stretch flex items-center justify-center pl-[6.12px] pr-[7px] relative shrink-0 size-[28px]" data-name="Listitem → Link">
      <Container304 />
    </div>
  );
}

function Container307() {
  return (
    <div className="relative shrink-0 size-[14px]" data-name="Container">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 14 14">
        <g id="Container">
          <path d={svgPaths.p174b8b00} fill="var(--fill-0, white)" id="Symbol" />
        </g>
      </svg>
    </div>
  );
}

function Container306() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0 size-[14px]" data-name="Container">
      <Container307 />
    </div>
  );
}

function ListitemLink4() {
  return (
    <div className="content-stretch flex items-center justify-center relative shrink-0 size-[28px]" data-name="Listitem → Link">
      <Container306 />
    </div>
  );
}

function List2() {
  return (
    <div className="content-stretch flex gap-[15px] items-start relative shrink-0 w-full" data-name="List">
      <ListitemLink />
      <ListitemLink1 />
      <ListitemLink2 />
      <ListitemLink3 />
      <ListitemLink4 />
    </div>
  );
}

function Container297() {
  return (
    <div className="content-stretch flex flex-col h-[28px] items-start relative shrink-0 w-full" data-name="Container">
      <List2 />
    </div>
  );
}

function Container290() {
  return (
    <div className="flex-[1_0_0] h-full min-w-px relative" data-name="Container">
      <div className="flex flex-col justify-center size-full">
        <div className="content-stretch flex flex-col items-start justify-center pb-[20px] pt-[15px] px-[15px] relative size-full">
          <Margin20 />
          <Margin21 />
          <Margin22 />
          <Container297 />
        </div>
      </div>
    </div>
  );
}

function Container289() {
  return (
    <div className="content-stretch flex items-start justify-center min-h-px relative self-stretch shrink-0 w-[439.99px]" data-name="Container">
      <Container290 />
    </div>
  );
}

function Container267() {
  return (
    <div className="content-stretch flex h-[278px] items-start max-w-[1320px] relative shrink-0 w-full" data-name="Container">
      <Container268 />
      <Container281 />
      <Container285 />
      <Container289 />
    </div>
  );
}

function Section13() {
  return (
    <div className="flex-[1_0_0] min-w-px relative self-stretch" data-name="Section">
      <div className="content-stretch flex flex-col items-start px-[50.4px] relative size-full">
        <Container267 />
      </div>
    </div>
  );
}

function Container266() {
  return (
    <div className="content-stretch flex h-[278px] items-start justify-center relative shrink-0 w-full" data-name="Container">
      <Section13 />
    </div>
  );
}

function Section12() {
  return (
    <div className="bg-black content-stretch flex flex-col items-start pb-[98px] pt-[80px] relative shrink-0 w-full" data-name="Section">
      <Container266 />
    </div>
  );
}

function Container313() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0 w-full" data-name="Container">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-white w-full">
        <p>
          <span className="leading-[21px]">{`Copyright © 2026 `}</span>
          <span className="font-['Jost:Regular',sans-serif] font-normal leading-[21px]">RisingBamboo.</span>
          <span className="leading-[21px]">{` All Rights Reserved.`}</span>
        </p>
      </div>
    </div>
  );
}

function Container312() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative" data-name="Container">
      <Container313 />
    </div>
  );
}

function Container311() {
  return (
    <div className="flex-[1_0_0] h-full min-w-px relative" data-name="Container">
      <div className="flex flex-row items-center justify-center size-full">
        <div className="content-stretch flex items-center justify-center p-[15px] relative size-full">
          <Container312 />
        </div>
      </div>
    </div>
  );
}

function Container310() {
  return (
    <div className="content-stretch flex flex-[1_0_0] items-start justify-center min-h-px min-w-px relative self-stretch" data-name="Container">
      <Container311 />
    </div>
  );
}

function Svg2() {
  return (
    <div className="relative shrink-0 size-[36px]" data-name="SVG">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 36 36">
        <g clipPath="url(#clip0_1_5708)" id="SVG">
          <path d={svgPaths.p21bd9700} fill="var(--fill-0, #21246E)" id="Vector" />
          <path d={svgPaths.p6584980} fill="var(--fill-0, white)" id="Vector_2" />
        </g>
        <defs>
          <clipPath id="clip0_1_5708">
            <rect fill="white" height="36" width="36" />
          </clipPath>
        </defs>
      </svg>
    </div>
  );
}

function Container318() {
  return (
    <div className="content-stretch flex items-start relative shrink-0" data-name="Container">
      <Svg2 />
    </div>
  );
}

function Container317() {
  return (
    <div className="content-stretch flex flex-col items-center pb-[7.2px] relative shrink-0" data-name="Container">
      <Container318 />
    </div>
  );
}

function Container316() {
  return (
    <div className="content-stretch flex flex-col items-start max-w-[660px] pr-[16px] relative shrink-0" data-name="Container">
      <Container317 />
    </div>
  );
}

function Svg3() {
  return (
    <div className="relative shrink-0 size-[36px]" data-name="SVG">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 36 36">
        <g id="SVG">
          <path d={svgPaths.p16fcace0} fill="var(--fill-0, #1C1C1C)" id="Vector" />
          <path d={svgPaths.p4cecc00} fill="var(--fill-0, #931B1D)" id="Vector_2" />
          <path d={svgPaths.p2af2100} fill="var(--fill-0, #F16622)" id="Vector_3" />
          <path d={svgPaths.p9755c80} fill="var(--fill-0, #F7981D)" id="Vector_4" />
          <path d={svgPaths.p33745000} fill="var(--fill-0, #CB2026)" id="Vector_5" />
          <path d={svgPaths.p3073e300} fill="var(--fill-0, white)" id="Vector_6" />
          <path d={svgPaths.p3c363400} fill="var(--fill-0, white)" id="Vector_7" />
          <path d={svgPaths.p20219200} fill="var(--fill-0, white)" id="Vector_8" />
          <path d={svgPaths.p3e55a880} fill="var(--fill-0, white)" id="Vector_9" />
          <path d={svgPaths.p257959c0} fill="var(--fill-0, white)" id="Vector_10" />
          <path d={svgPaths.p3c6e1960} fill="var(--fill-0, white)" id="Vector_11" />
          <path d={svgPaths.p9437c40} fill="var(--fill-0, white)" id="Vector_12" />
          <path d={svgPaths.p23e0f600} fill="var(--fill-0, white)" id="Vector_13" />
          <path d={svgPaths.p16a85b00} fill="var(--fill-0, white)" id="Vector_14" />
          <path d={svgPaths.p78df980} fill="var(--fill-0, white)" id="Vector_15" />
        </g>
      </svg>
    </div>
  );
}

function Container321() {
  return (
    <div className="content-stretch flex items-start relative shrink-0" data-name="Container">
      <Svg3 />
    </div>
  );
}

function Container320() {
  return (
    <div className="content-stretch flex flex-col items-center pb-[7.2px] relative shrink-0" data-name="Container">
      <Container321 />
    </div>
  );
}

function Container319() {
  return (
    <div className="content-stretch flex flex-col items-start max-w-[660px] pr-[16px] relative shrink-0" data-name="Container">
      <Container320 />
    </div>
  );
}

function Svg4() {
  return (
    <div className="relative shrink-0 size-[36px]" data-name="SVG">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 36 36">
        <g id="SVG">
          <path d={svgPaths.p2fb10c80} fill="var(--fill-0, #306FC5)" id="Vector" />
          <path d={svgPaths.p16bd8e80} fill="var(--fill-0, white)" id="Vector_2" />
          <path d={svgPaths.p32dcdbe0} fill="var(--fill-0, white)" id="Vector_3" />
          <path d={svgPaths.p2dca2980} fill="var(--fill-0, white)" id="Vector_4" />
          <path d={svgPaths.p39b45880} fill="var(--fill-0, white)" id="Vector_5" />
          <path d={svgPaths.p2471bd80} fill="var(--fill-0, white)" id="Vector_6" />
          <path d={svgPaths.p18f81180} fill="var(--fill-0, white)" id="Vector_7" />
          <path d={svgPaths.p3a0a9300} fill="var(--fill-0, white)" id="Vector_8" />
          <path d={svgPaths.p35dd1f00} fill="var(--fill-0, white)" id="Vector_9" />
          <path d={svgPaths.p16ffa180} fill="var(--fill-0, white)" id="Vector_10" />
        </g>
      </svg>
    </div>
  );
}

function Container324() {
  return (
    <div className="content-stretch flex items-start relative shrink-0" data-name="Container">
      <Svg4 />
    </div>
  );
}

function Container323() {
  return (
    <div className="content-stretch flex flex-col items-center pb-[7.2px] relative shrink-0" data-name="Container">
      <Container324 />
    </div>
  );
}

function Container322() {
  return (
    <div className="content-stretch flex flex-col items-start max-w-[660px] pr-[16px] relative shrink-0" data-name="Container">
      <Container323 />
    </div>
  );
}

function Svg5() {
  return (
    <div className="relative shrink-0 size-[36px]" data-name="SVG">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 36 36">
        <g id="SVG">
          <path d={svgPaths.p11507b80} fill="var(--fill-0, white)" id="Vector" />
          <path d={svgPaths.p27479800} fill="var(--fill-0, #2196F3)" id="Vector_2" />
          <path d={svgPaths.p223cea60} fill="var(--fill-0, #1976D2)" id="Vector_3" />
        </g>
      </svg>
    </div>
  );
}

function Container327() {
  return (
    <div className="content-stretch flex items-start relative shrink-0" data-name="Container">
      <Svg5 />
    </div>
  );
}

function Container326() {
  return (
    <div className="content-stretch flex flex-col items-center pb-[7.2px] relative shrink-0" data-name="Container">
      <Container327 />
    </div>
  );
}

function Container325() {
  return (
    <div className="content-stretch flex flex-col items-start max-w-[660px] pr-[16px] relative shrink-0" data-name="Container">
      <Container326 />
    </div>
  );
}

function Svg6() {
  return (
    <div className="relative shrink-0 size-[36px]" data-name="SVG">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 36 36">
        <g id="SVG">
          <path d={svgPaths.p11507b80} fill="var(--fill-0, white)" id="Vector" />
          <path clipRule="evenodd" d={svgPaths.p1c8c3800} fill="var(--fill-0, #227FBB)" fillRule="evenodd" id="Vector_2" opacity="0.938" />
          <path clipRule="evenodd" d={svgPaths.pea1f180} fill="var(--fill-0, #227FBB)" fillRule="evenodd" id="Vector_3" opacity="0.938" />
          <path clipRule="evenodd" d={svgPaths.p2ef07e9c} fill="var(--fill-0, #227FBB)" fillRule="evenodd" id="Vector_4" opacity="0.938" />
          <path clipRule="evenodd" d={svgPaths.pe26a700} fill="var(--fill-0, #227FBB)" fillRule="evenodd" id="Vector_5" opacity="0.938" />
          <path clipRule="evenodd" d={svgPaths.p3b852b80} fill="var(--fill-0, #227FBB)" fillRule="evenodd" id="Vector_6" opacity="0.938" />
          <path clipRule="evenodd" d={svgPaths.p306a3380} fill="var(--fill-0, #1AAF5D)" fillRule="evenodd" id="Vector_7" />
          <path clipRule="evenodd" d={svgPaths.p2562c1c0} fill="var(--fill-0, #F59D00)" fillRule="evenodd" id="Vector_8" />
        </g>
      </svg>
    </div>
  );
}

function Container330() {
  return (
    <div className="content-stretch flex items-start relative shrink-0" data-name="Container">
      <Svg6 />
    </div>
  );
}

function Container329() {
  return (
    <div className="content-stretch flex flex-col items-center pb-[7.2px] relative shrink-0" data-name="Container">
      <Container330 />
    </div>
  );
}

function Container328() {
  return (
    <div className="content-stretch flex flex-col items-start max-w-[660px] relative shrink-0" data-name="Container">
      <Container329 />
    </div>
  );
}

function Container315() {
  return (
    <div className="flex-[1_0_0] h-full min-w-px relative" data-name="Container">
      <div className="flex flex-row items-center justify-end size-full">
        <div className="content-stretch flex items-center justify-end p-[15px] relative size-full">
          <Container316 />
          <Container319 />
          <Container322 />
          <Container325 />
          <Container328 />
        </div>
      </div>
    </div>
  );
}

function Container314() {
  return (
    <div className="content-stretch flex flex-[1_0_0] items-start justify-center min-h-px min-w-px relative self-stretch" data-name="Container">
      <Container315 />
    </div>
  );
}

function Container309() {
  return (
    <div className="content-stretch flex h-[73.2px] items-start max-w-[1320px] relative shrink-0 w-full" data-name="Container">
      <Container310 />
      <Container314 />
    </div>
  );
}

function Section15() {
  return (
    <div className="flex-[1_0_0] min-w-px relative self-stretch" data-name="Section">
      <div className="content-stretch flex flex-col items-start py-[15px] relative size-full">
        <Container309 />
      </div>
    </div>
  );
}

function Container308() {
  return (
    <div className="h-[103.2px] max-w-[1320px] relative shrink-0 w-full" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid content-stretch flex items-start justify-center max-w-[inherit] relative size-full">
        <Section15 />
      </div>
    </div>
  );
}

function Section14() {
  return (
    <div className="bg-black relative shrink-0 w-full" data-name="Section">
      <div aria-hidden="true" className="absolute border-[rgba(255,255,255,0.1)] border-solid border-t-[0.8px] inset-0 pointer-events-none" />
      <div className="content-stretch flex flex-col items-start pt-[0.8px] px-[50.4px] relative size-full">
        <Container308 />
      </div>
    </div>
  );
}

function Container265() {
  return (
    <div className="absolute content-stretch flex flex-col h-[560px] items-start left-0 right-0 top-[6247px]" data-name="Container">
      <Section12 />
      <Section14 />
    </div>
  );
}

function Link58() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px pr-[14px] relative" data-name="Link">
      <div className="flex flex-col font-['Oswald:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[12px] text-black uppercase whitespace-nowrap">
        <p className="leading-[24px]">HOME</p>
      </div>
      <div className="absolute h-[2.997px] left-[36.18px] top-[10.63px] w-[6px]" data-name="Symbol">
        <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 6 2.9973">
          <path d={svgPaths.p3d2f8a40} fill="var(--fill-0, black)" id="Symbol" />
        </svg>
      </div>
    </div>
  );
}

function Item12() {
  return (
    <div className="content-stretch flex flex-col h-[24px] items-start justify-center pr-[20px] relative shrink-0" data-name="Item">
      <Link58 />
    </div>
  );
}

function Link59() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px pr-[14px] relative" data-name="Link">
      <div className="flex flex-col font-['Oswald:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#909090] text-[12px] uppercase whitespace-nowrap">
        <p className="leading-[24px]">SHOP</p>
      </div>
      <div className="absolute h-[2.997px] left-[35.37px] top-[10.63px] w-[6px]" data-name="Symbol">
        <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 6 2.9973">
          <path d={svgPaths.p3d2f8a40} fill="var(--fill-0, #909090)" id="Symbol" />
        </svg>
      </div>
    </div>
  );
}

function Item13() {
  return (
    <div className="content-stretch flex flex-col h-[24px] items-start justify-center px-[20px] relative shrink-0" data-name="Item">
      <Link59 />
    </div>
  );
}

function Link60() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px pr-[13.99px] relative" data-name="Link">
      <div className="flex flex-col font-['Oswald:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#909090] text-[12px] uppercase whitespace-nowrap">
        <p className="leading-[24px]">PRODUCT</p>
      </div>
      <div className="absolute h-[2.997px] left-[55.18px] top-[10.63px] w-[6px]" data-name="Symbol">
        <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 6 2.9973">
          <path d={svgPaths.p3d2f8a40} fill="var(--fill-0, #909090)" id="Symbol" />
        </svg>
      </div>
    </div>
  );
}

function Item14() {
  return (
    <div className="content-stretch flex flex-col h-[24px] items-start justify-center pl-[20px] pr-[20.01px] relative shrink-0" data-name="Item">
      <Link60 />
    </div>
  );
}

function Link61() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px pr-[14px] relative" data-name="Link">
      <div className="flex flex-col font-['Oswald:Bold',sans-serif] font-bold justify-center leading-[0] relative shrink-0 text-[#909090] text-[12px] uppercase whitespace-nowrap">
        <p className="leading-[24px]">BLOG</p>
      </div>
      <div className="absolute h-[2.997px] left-[34.23px] top-[10.63px] w-[6px]" data-name="Symbol">
        <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 6 2.9973">
          <path d={svgPaths.p3d2f8a40} fill="var(--fill-0, #909090)" id="Symbol" />
        </svg>
      </div>
    </div>
  );
}

function Item15() {
  return (
    <div className="content-stretch flex flex-col h-[24px] items-start justify-center px-[20px] relative shrink-0" data-name="Item">
      <Link61 />
    </div>
  );
}

function List3() {
  return (
    <div className="content-stretch flex items-start relative shrink-0" data-name="List">
      <Item12 />
      <Item13 />
      <Item14 />
      <Item15 />
    </div>
  );
}

function Nav() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0" data-name="Nav">
      <List3 />
    </div>
  );
}

function Container335() {
  return (
    <div className="content-stretch flex flex-col items-start justify-center relative shrink-0 w-full" data-name="Container">
      <Nav />
    </div>
  );
}

function Container334() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0 w-[324.97px]" data-name="Container">
      <Container335 />
    </div>
  );
}

function Container333() {
  return (
    <div className="content-stretch flex flex-[1_0_0] items-center min-w-px relative" data-name="Container">
      <Container334 />
    </div>
  );
}

function Logo() {
  return (
    <div className="h-[24.19px] relative shrink-0 w-[124px]" data-name="logo">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img alt="" className="absolute h-[100.02%] left-0 max-w-none top-[-0.01%] w-full" src={imgLogo} />
      </div>
    </div>
  );
}

function Link62() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0" data-name="Link">
      <Logo />
    </div>
  );
}

function Container337() {
  return (
    <div className="content-stretch flex items-center relative shrink-0" data-name="Container">
      <Link62 />
    </div>
  );
}

function Container336() {
  return (
    <div className="content-stretch flex items-center justify-center relative shrink-0" data-name="Container">
      <Container337 />
    </div>
  );
}

function Container343() {
  return (
    <div className="content-stretch flex flex-col items-start overflow-clip pr-[89.82px] relative shrink-0" data-name="Container">
      <div className="flex flex-col font-['Jost:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[#6b7280] text-[13px] whitespace-nowrap">
        <p className="leading-[normal]">Search</p>
      </div>
    </div>
  );
}

function Input1() {
  return (
    <div className="bg-white content-stretch flex flex-col h-[44px] items-start overflow-clip pl-[20px] pr-[52px] py-[12.8px] relative rounded-[44px] shadow-[2px_2px_6px_0px_rgba(0,0,0,0.2)] shrink-0" data-name="Input">
      <Container343 />
    </div>
  );
}

function Svg7() {
  return (
    <div className="relative shrink-0 size-[20px]" data-name="SVG">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 20 20">
        <g id="SVG">
          <path d={svgPaths.p13b5d880} fill="var(--fill-0, black)" id="Vector" />
        </g>
      </svg>
    </div>
  );
}

function Button1() {
  return (
    <div className="-translate-y-1/2 absolute content-stretch flex items-center justify-center right-[1.62px] rounded-[40px] size-[40px] top-1/2" data-name="Button">
      <Svg7 />
    </div>
  );
}

function Container342() {
  return (
    <div className="content-stretch flex items-center justify-center relative self-stretch shrink-0" data-name="Container">
      <Input1 />
      <Button1 />
    </div>
  );
}

function Search() {
  return (
    <div className="content-stretch flex h-[44px] items-start justify-center relative shrink-0" data-name="Search">
      <Container342 />
    </div>
  );
}

function Container341() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0" data-name="Container">
      <Search />
    </div>
  );
}

function Container340() {
  return (
    <div className="content-stretch flex items-center justify-center relative shrink-0" data-name="Container">
      <Container341 />
    </div>
  );
}

function Container339() {
  return (
    <div className="content-stretch flex items-center justify-end relative shrink-0 w-full" data-name="Container">
      <Container340 />
    </div>
  );
}

function Container338() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-w-px relative" data-name="Container">
      <Container339 />
    </div>
  );
}

function Container332() {
  return (
    <div className="relative shrink-0 w-full" data-name="Container">
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center justify-between px-[30px] py-[18px] relative size-full">
          <Container333 />
          <Container336 />
          <Container338 />
        </div>
      </div>
    </div>
  );
}

function Container331() {
  return (
    <div className="content-stretch flex flex-col h-[80px] items-start relative shrink-0 w-full" data-name="Container">
      <Container332 />
    </div>
  );
}

function Header() {
  return (
    <div className="absolute content-stretch flex flex-col items-start left-0 min-h-[80px] top-0 w-full" data-name="Header">
      <Container331 />
    </div>
  );
}

function Container12() {
  return (
    <div className="h-[6807px] relative shrink-0 w-full max-w-[1421px]" data-name="Container">
      <Masthead />
      <Container265 />
      <Header />
    </div>
  );
}

function Svg8() {
  return (
    <div className="absolute left-0 size-[46px] top-0" data-name="SVG">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 46 46">
        <g clipPath="url(#clip0_1_5678)" id="SVG">
          <path d={svgPaths.p375dda80} id="Vector" stroke="var(--stroke-0, #0DB22A)" strokeDasharray="138.87 138.87" strokeWidth="1.80392" />
        </g>
        <defs>
          <clipPath id="clip0_1_5678">
            <rect fill="white" height="46" width="46" />
          </clipPath>
        </defs>
      </svg>
    </div>
  );
}

function Container344() {
  return (
    <div className="relative shrink-0 size-[15px]" data-name="Container">
      <div className="absolute inset-[-2.62%_0_0_0]">
        <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15 15.3924">
          <g id="Container">
            <path d={svgPaths.p2f3c8f80} fill="var(--fill-0, black)" id="Symbol" />
          </g>
        </svg>
      </div>
    </div>
  );
}

function Background12() {
  return (
    <button className="bg-white content-stretch cursor-pointer flex items-center justify-center p-[12px] pointer-events-auto rounded-[9999px] size-[46px] sticky top-0" data-name="Background">
      <div className="absolute bottom-0 pointer-events-none right-0 rounded-[9999px] size-[46px]" data-name="Overlay+Shadow">
        <div aria-hidden="true" className="absolute bg-[rgba(255,255,255,0)] inset-0 rounded-[9999px]" />
        <div className="absolute inset-0 rounded-[inherit] shadow-[inset_0px_0px_0px_2px_rgba(34,34,34,0.2)]" />
      </div>
      <Svg8 />
      <Container344 />
    </button>
  );
}

export default function TrangCh() {
  return (
    <div className="bg-white content-stretch flex flex-col items-start relative w-full overflow-hidden" data-name="Trang chủ">
      <Container12 />

    </div>
  );
}